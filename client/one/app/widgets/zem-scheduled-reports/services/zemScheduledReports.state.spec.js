describe('zemScheduledReportsStateService', function () {
    ...
    beforeEach(inject(function (_$injector_) {
        ...
        zemScheduledReportsStateService = $injector.get('zemScheduledReportsStateService');
        zemScheduledReportsEndpoint = $injector.get('zemScheduledReportsEndpoint');
    ...
    it('should correctly update state when successfully getting data from endpoint', function () {
        ...
        var stateService = zemScheduledReportsStateService.createInstance({id: 999, type: 'mockedAccount'});
        var mockedAsyncFunction = zemSpecsHelper.getMockedAsyncFunction($injector, reports);

        spyOn(zemScheduledReportsEndpoint, 'list').and.callFake(mockedAsyncFunction);

        stateService.reloadReports();
        expect(stateService.getState()).toEqual({loadReportsRequestInProgress: true});
        $rootScope.$digest();
    ...
    it('should correctly update state when unsuccessfully getting data from endpoint', function () {
        var stateService = zemScheduledReportsStateService.createInstance({id: 999, type: 'mockedAccount'});
        var mockedAsyncFunction = zemSpecsHelper.getMockedAsyncFunction($injector, 'error', true);
    ...
    it('should make a correct request to endpoint when removing report and set error message on failure', function () {
        var stateService = zemScheduledReportsStateService.createInstance({id: 999, type: 'mockedAccount'});
        var mockedAsyncFunction = zemSpecsHelper.getMockedAsyncFunction($injector, {}, true);
        ...
        expect(stateService.getState()).toEqual({
            removeErrorMessage: 'Error removing report. Please contact support.',
