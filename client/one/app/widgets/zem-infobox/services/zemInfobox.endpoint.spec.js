describe('zemInfoboxEndpoint', function () {
    ...
    it('should make correct request to server for all accounts', function () {
        ...
        spyOn(zemDataFilterService, 'getFilteredAgencies').and.returnValue(filteredAgencies);
        spyOn(zemDataFilterService, 'getFilteredAccountTypes').and.returnValue(filteredAccountTypes);
    ...
    it('should make correct request to server for ad group entity', function () {
        ...
        spyOn($http, 'get').and.callFake(mockedAsyncFunction);
        spyOn(zemDataFilterService, 'getDateRange').and.returnValue(dateRange);
        spyOn(zemDataFilterService, 'getFilteredSources').and.returnValue(filteredSources);

        zemInfoboxEndpoint.getInfoboxData(entity);
        expect($http.get).toHaveBeenCalledWith(
            '/api/ad_groups/999/overview/',
            {
                params: {
                    start_date: dateRange.startDate.format(),
                    end_date: dateRange.endDate.format(),
