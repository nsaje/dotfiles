angular.module('one.widgets').service('zemGridIntegrationSelectionService', function (zemSelectionService, zemGridConstants, zemGridEndpointColumns) { // eslint-disable-line
    ...
    function createGridSelection (gridApi) {
        ...
        gridSelection = setGridSelectedAndUnselectedRows(gridSelection, rows);
        gridSelection = addFooterRowToGridSelection(gridApi, gridSelection, rows);
    ...
    function setGridSelectionFilter (gridApi) {
        ...
        if (zemSelectionService.isAllSelected()) {
            gridApi.setSelectionFilter(zemGridConstants.gridSelectionFilterType.ALL);
        } else if (selectedBatch && metaData && metaData.ext && metaData.ext.batches) {
            var batchSelectionFilter = getBatchSelectionFilter(selectedBatch, metaData.ext.batches);
            if (batchSelectionFilter) {
                gridApi.setSelectionFilter(zemGridConstants.gridSelectionFilterType.CUSTOM, batchSelectionFilter);
            }
        } else {
            gridApi.setSelectionFilter(zemGridConstants.gridSelectionFilterType.NONE);
        }
    }
    ...
    function setGridSelectedAndUnselectedRows (gridSelection, rows) {
        ...
        rows.forEach(function (row) {
            if (row.level === zemGridConstants.gridRowLevel.BASE &&
    ...
    function addFooterRowToGridSelection (gridApi, gridSelection, rows) {
        ...
        if (footerRow && (zemSelectionService.isTotalsSelected() || gridApi.isSelectionEmpty())) {
        ...
        } else if (footerRow && !zemSelectionService.isTotalsSelected() &&
                   (zemSelectionService.isAllSelected() || zemSelectionService.getSelectedBatch())) {
