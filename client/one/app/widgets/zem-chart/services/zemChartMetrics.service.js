...
angular.module('one.widgets').factory('zemChartMetricsService', function () {
    function insertAcquisitionColumns (columns, position, isShown, isInternal) {
        columns.splice(position, 0, {
            ...
            shown: isShown,
            internal: isInternal,
            help: 'Total number of sessions within a date range. A session is the period of time ' +
                  'in which a user is actively engaged with your site.',
            totalRow: true,
            order: true,
            initialOrder: 'desc',
        }, {
            ...
            shown: isShown,
            internal: isInternal,
            help: 'Total number of pageviews made during the selected date range. ' +
                  'A pageview is a view of a single page. Repeated views are counted.',
            totalRow: true,
            order: true,
            initialOrder: 'desc',
        });
    ...
    function insertConversionsGoalChartOptions (chartOptions, conversionGoals) {
        ...
        var conversionsPosition = findChartOptionPosition(chartOptions,
            constants.chartMetric.CONVERSION_GOALS_PLACEHOLDER);
        Array.prototype.splice.apply(chartOptions, [conversionsPosition, 0].concat(newOptions));

        var goalsPosition = findChartOptionPosition(chartOptions,
            constants.chartMetric.CONVERSION_GOALS_AVG_COST_PLACEHOLDER);
