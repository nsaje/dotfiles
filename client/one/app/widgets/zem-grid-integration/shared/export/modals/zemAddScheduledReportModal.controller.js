...
//
angular.module('one.widgets').controller('zemAddScheduledReportModalCtrl', function ($scope, $q, $http, zemAddScheduledReportModalEndpoint, zemDataFilterService) {  // eslint-disable-line max-len
    ...
    $scope.addScheduledReport = function () {
        ...
        zemAddScheduledReportModalEndpoint.put(url, data).then(
            function () {
                $scope.$close();
            },
            function (errors) {
...
angular.module('one.widgets').service('zemAddScheduledReportModalEndpoint', function ($q, $http) {
    this.put = put;
