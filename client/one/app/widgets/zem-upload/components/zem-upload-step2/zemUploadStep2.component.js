...
angular.module('one.widgets').controller('ZemUploadStep2Ctrl', function ($scope, config, $interval, $window, $timeout) {
    var vm = this;
    vm.config = config;
    ...
    function isAnyVideoAssetBeingProcessed () {
        for (var i = 0; i < vm.candidates.length; i++) {
            if (vm.adPickerApi.isVideoAssetBeingProcessed(vm.candidates[i])) {
                return true;
            }
        }
        return false;
    ...
    function areAnyVideoAssetProcessingErrorsPresent () {
        for (var i = 0; i < vm.candidates.length; i++) {
            if (vm.adPickerApi.isVideoAssetProcessingErrorPresent(vm.candidates[i])) {
                return true;
            }
        }
        return false;
    ...
    vm.isLoading = function () {
        return vm.adPickerApi.getWaitingCandidateIds().length ||
