angular.module('one.widgets').service('zemCreateEntityActionService', function ($q, $state, zemEntityService, zemNavigationService, zemNavigationNewService, zemUploadService) { // eslint-disable-line max-len
    ...
    function createEntity (entityType, parent) {
        ...
        return zemEntityService.createEntity(entityType, parentId).then(function (entity) {
            entity.type = entityType;
            reloadCache(parent, entity);
            zemNavigationNewService.navigateTo(entity, {settings: 'create'});
        }, function (err) {
            return $q.reject(err);
        });
    }
