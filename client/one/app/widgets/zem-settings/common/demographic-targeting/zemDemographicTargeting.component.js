angular.module('one.widgets').component('zemDemographicTargeting', {
    ...
    controller: function (zemUtils, zemDemographicTaxonomyService, zemDemographicTargetingConstants, zemDemographicTargetingStateService) { // eslint-disable-line max-len
        ...
        function initialize () {
            $ctrl.stateService = zemDemographicTargetingStateService.createInstance($ctrl.entity);
            $ctrl.stateService.initialize();
            $ctrl.state = $ctrl.stateService.getState();
        }

        function enable () {
            var root = $ctrl.stateService.createNode(zemDemographicTargetingConstants.EXPRESSION_TYPE.AND);
            $ctrl.stateService.createNode(zemDemographicTargetingConstants.EXPRESSION_TYPE.OR, root);
        }

        function addExclusion () {
            var notNode = $ctrl.stateService.createNode(
                zemDemographicTargetingConstants.EXPRESSION_TYPE.NOT, $ctrl.state.expressionTree);
            $ctrl.stateService.createNode(zemDemographicTargetingConstants.EXPRESSION_TYPE.OR, notNode);
        }

        function addInclusion () {
            $ctrl.stateService.createNode(
                zemDemographicTargetingConstants.EXPRESSION_TYPE.OR, $ctrl.state.expressionTree);
        }
        ...
        function canAddExclusion () {
            ...
            return tree.childNodes.filter(function (node) {
                return node.type === zemDemographicTargetingConstants.EXPRESSION_TYPE.NOT;
            }).length < 1;
        }
        ...
        function canAddInclusion () {
            ...
            return tree.childNodes.filter(function (node) {
                return node.type === zemDemographicTargetingConstants.EXPRESSION_TYPE.OR;
            }).length < 2;
        }
    },
});
