...
// NOTE: `vs-repeat` directive from angular-vs-repeat library was added to Z1 code base in order to make it compatible
// with Angular 2 and above. Directive caused 100% CPU usage because it called `requestAnimationFrame` inside a
// `$scope.watch()` callback. This caused too many NgZones observed by Angular to be created. Issue was fixed by
// wrapping `requestAnimationFrame` calls in `NgZone.runOutsideAngular()` (`NgZone` is a core Angular module (from
// `@angular/core`) downgraded to work with AngularJS).
...
(function(window, angular) {
    ...
    function getScrollOffset(vsElement, scrollElement, isHorizontal) {
        var vsPos = vsElement.getBoundingClientRect()[
    ...
    var vsRepeatModule = angular.module('one.common').directive('vsRepeat', [
        '$compile',
        '$parse',
        'NgZone',
        function($compile, $parse, NgZone) {
            return {
                restrict: 'A',
                scope: true,
                compile: function($element, $attrs) {
                    var repeatContainer = angular.isDefined(
                            $attrs.vsRepeatContainer
                        )
                            ? angular.element(
                    ...
                    if (ngRepeatChild.attr('ng-repeat')) {
                        ...
                        ngRepeatExpression = ngRepeatChild.attr(
                            'data-ng-repeat'
                        );
                        ...
                        throw new Error(
                            'angular-vs-repeat: no ng-repeat directive on a child element'
                        );
                    }

                    expressionMatches = /^\s*(\S+)\s+in\s+([\S\s]+?)(track\s+by\s+\S+)?$/.exec(
                        ngRepeatExpression
                    );
                    ...
                    if (isNgRepeatStart) {
                        ...
                        while (
                            repeaterElement.attr('ng-repeat-end') == null &&
                            repeaterElement.attr('data-ng-repeat-end') == null
                        ) {
                            index++;
                            repeaterElement = repeatContainer
                                .children()
                                .eq(index);
                    ...
                    return {
                        pre: function($scope, $element, $attrs) {
                            var repeatContainer = angular.isDefined(
                                ...
                                originalLength,
                                $$horizontal =
                                    typeof $attrs.vsHorizontal !== 'undefined',
                                $beforeContent = angular.element(
                                    '<' +
                                        childTagName +
                                        ' class="vs-repeat-before-content"></' +
                                        childTagName +
                                        '>'
                                ),
                                $afterContent = angular.element(
                                    '<' +
                                        childTagName +
                                ...
                                sizesPropertyExists =
                                    !!$attrs.vsSize || !!$attrs.vsSizeProperty,
                                $scrollParent = $attrs.vsScrollParent
                                    ? $attrs.vsScrollParent === 'window'
                                ...
                                $$options =
                                    'vsOptions' in $attrs
                                        ? $scope.$eval($attrs.vsOptions)
                                        : {},
                                clientSize = $$horizontal
                                    ? 'clientWidth'
                                    : 'clientHeight',
                                offsetSize = $$horizontal
                                    ? 'offsetWidth'
                                    : 'offsetHeight',
                            ...
                            if (
                                !('vsSize' in $attrs) &&
                                'vsSizeProperty' in $attrs
                            ) {
                                console.warn(
                                    'vs-size-property attribute is deprecated. Please use vs-size attribute which also accepts angular expressions.'
                                );
                            }
                            ...
                            $scope.elementSize =
                                +$attrs.vsRepeat ||
                                getClientSize($scrollParent[0], clientSize) ||
                                50;
                            ...
                            Object.keys(attributesDictionary).forEach(function(
                                key
                            ) {
                                if ($attrs[key]) {
                            ...
                            function refresh() {
                                if (
                                    ...
                                    if (sizesPropertyExists) {
                                        $scope.sizes = originalCollection.map(
                                            function(item) {
                                        ...
                                        var sum = 0;
                                        $scope.sizesCumulative = $scope.sizes.map(
                                            function(size) {
                                                var res = sum;
                                                sum += size;
                                                return res;
                            ...
                            function setAutoSize() {
                                if (autoSize) {
                                    $scope.$$postDigest(function() {
                                        if (
                                            repeatContainer[0].offsetHeight ||
                                            repeatContainer[0].offsetWidth
                                        ) {
                                            // element is visible
                            ...
                            function getLayoutProp() {
                                var layoutPropPrefix =
                                    childTagName === 'tr' ? '' : 'min-';
                                var layoutProp = $$horizontal
                                    ? layoutPropPrefix + 'width'
                                    : layoutPropPrefix + 'height';
                                return layoutProp;
                            }
                            ...
                            function onWindowResize() {
                                if (
                                    typeof $attrs.vsAutoresize !== 'undefined'
                                ) {
                                    autoSize = true;
                                    setAutoSize();
                            ...
                            $scope.$on('vsRenderAll', function() {
                                ...
                                if ($$options.latch) {
                                    setTimeout(function() {
                                        ...
                                        _maxEndIndex = Math.max(
                                            __endIndex,
                                            _maxEndIndex
                                        );
                                        $scope.endIndex = $$options.latch
                                            ? _maxEndIndex
                                            : __endIndex;
                                        $scope[
                                            collectionName
                                        ] = originalCollection.slice(
                                            $scope.startIndex,
                                            $scope.endIndex
                                        );
                            ...
                            function updateInnerCollection() {
                                ...
                                var scrollOffset =
                                    repeatContainer[0] === $scrollParent[0]
                                        ? 0
                                        : getScrollOffset(
                                              repeatContainer[0],
                                              $scrollParent[0],
                                ...
                                if (sizesPropertyExists) {
                                    __startIndex = 0;
                                    while (
                                        $scope.sizesCumulative[__startIndex] <
                                        $scrollPosition -
                                            $scope.offsetBefore -
                                            scrollOffset
                                    ) {
                                    ...
                                    while (
                                        $scope.sizesCumulative[__endIndex] <
                                        $scrollPosition -
                                            $scope.offsetBefore -
                                            scrollOffset +
                                            $clientSize
                                ...
                                $scope.startIndex = $$options.latch
                                    ? _minStartIndex
                                    : __startIndex;
                                $scope.endIndex = $$options.latch
                                    ? _maxEndIndex
                                ...
                                if (digestRequired) {
                                    ...
                                    if ($attrs.vsScrolledToEnd) {
                                        var triggerIndex =
                                            ...
                                            ($scope.scrolledToEndOffset || 0);
                                        if (
                                            ($scope.endIndex >= triggerIndex &&
                                                _prevEndIndex < triggerIndex) ||
                                            (originalCollection.length &&
                                                $scope.endIndex ===
                                                    originalCollection.length)
                                        ) {
                                            $scope.$eval(
                                                $attrs.vsScrolledToEnd
                                            );
                                        }
                                    }
