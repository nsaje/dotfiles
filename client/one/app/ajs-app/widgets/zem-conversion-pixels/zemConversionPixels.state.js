angular
    .module('one.widgets')
    .service('zemConversionPixelsStateService', function(
        $q,
        $http,
        zemPubSubService,
        ...
        function StateService(account) {
            ...
            function initialize() {
                filteredStatusesUpdateHandler = zemDataFilterService.onFilteredStatusesUpdate(
                    function() {
                        state.conversionPixels = conversionPixelsCache.filter(
                            filterPixelsByStatus
                        );
                    }
                );
            ...
            function create(pixel) {
                ...
                return zemConversionPixelsEndpoint
                    ...
                    .then(
                        ...
                        function(data) {
                            if (data && data.errors) {
                                state.requests.create.validationErrors =
                                    data.errors;
                            } else {
            ...
            function archive(conversionPixel) {
                ...
                return zemConversionPixelsEndpoint
                    .archive(conversionPixel)
                    .then(
                        function(data) {
                            conversionPixel.archived = data.archived;
                        },
                        function(data) {
                            state.requests.archive[conversionPixel.id].error =
                                data.errors;
                            $q.reject();
                        }
                    )
            ...
            function restore(conversionPixel) {
                ...
                return zemConversionPixelsEndpoint
                    .restore(conversionPixel)
                    .then(
                        function(data) {
                            conversionPixel.archived = data.archived;
                        },
                        function(data) {
                            state.requests.restore[conversionPixel.id].error =
                                data.errors;
                            $q.reject();
                        }
                    )
                    .finally(function() {
                        state.requests.restore[
                            conversionPixel.id
                        ].inProgress = false;
                    });
            }
            ...
            function getPixelById(id) {
                return conversionPixelsCache.filter(function(pixel) {
                    return pixel.id === id;
                })[0];
            }
