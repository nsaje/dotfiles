angular.module('one.widgets').service('zemAdGroupSourcesStateService', function ($q, $http, $state, zemUserService, zemDataFilterService, zemAdGroupSourcesEndpoint) { //eslint-disable-line max-len
    ...
    function StateService (adGroup) {
        ...
        function create (sourceId) {
            ...
            return zemAdGroupSourcesEndpoint.create(adGroup.id, sourceId).then(
                function () {
                    $state.reload();
                },
                function (err) {
        ...
        function list () {
            ...
            zemAdGroupSourcesEndpoint.list(adGroup.id, config).then(
                function (data) {
                    state.sources = [];
                    for (var source, i = 0; i < data.sources.length; i++) {
                        ...
                        if (!source.canTargetExistingRegions) {
                            notificationMsg = source.name + ' doesn\'t support DMA targeting. ' +
                                'Turn off DMA targeting to add ' + source.name + '.';
                        }
                        if (!source.canRetarget) {
                            notificationMsg = (notificationMsg ? notificationMsg + ' ' : '') +
                                source.name + ' doesn\'t support retargeting. ' +
                                'Turn off retargeting to add ' + source.name + '.';
                        }
                        state.sources.push({
                            name: source.name,
                            value: source.id,
