angular.module('one.widgets').service('zemCustomAudiencesStateService', function ($q, $http, zemDataFilterService, zemPubSubService, zemCustomAudiencesEndpoint, zemConversionPixelsEndpoint) { //eslint-disable-line max-len
    ...
    function StateService (account) {
        ...
        //
        function initialize () {
            filteredStatusesUpdateHandler = zemDataFilterService.onFilteredStatusesUpdate(list);
        ...
        function create (audienceData) {
            ...
            return zemCustomAudiencesEndpoint.post(account.id, audienceData).then(
                function (data) {
        ...
        function update (audienceId, data) {
            ...
            return zemCustomAudiencesEndpoint.put(account.id, audienceId, data).then(
                function () {
                    list(); // refresh audiences list
                },
                function (errors) {
                    state.requests.update[audienceId].validationErrors = errors;
                    return $q.reject(errors);
                }
            ).finally(function () {
        ...
        function get (audienceId) {
            ...
            return zemCustomAudiencesEndpoint.get(account.id, audienceId).then(
                function (data) {
        ...
        function list () {
            ...
            return zemCustomAudiencesEndpoint.list(account.id, showArchived, true).then(
                function (data) {
        ...
        function archive (audience) {
            ...
            return zemCustomAudiencesEndpoint.archive(account.id, audience.id).then(
                function () {
        ...
        function restore (audience) {
            ...
            return zemCustomAudiencesEndpoint.restore(account.id, audience.id).then(
                function () {
