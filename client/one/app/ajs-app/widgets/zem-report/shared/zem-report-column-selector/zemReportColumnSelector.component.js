...
angular.module('one.widgets').component('zemReportColumnSelector', {
    ...
    controller: function ($element, $timeout, zemCostModeService) {
        ...
        function onToggleColumns (newColumnsState) {
            setVisibleColumns($ctrl.api.getTogglableColumns(clonedColumns), newColumnsState);
        }
        ...
        function setVisibleColumns (toggledColumns, visible, allColumns) {
            ...
            var columnsToToggle = $ctrl.api.getColumnsToToggle(toggledColumns, allColumns);
            columnsToToggle.forEach(function (column) {
                column.visible = visible;
            });

            // Reinitialize $ctrl.categories with updated clonedColumns
            $ctrl.categories = $ctrl.api.getCategorizedColumns(zemCostModeService, clonedColumns);

            $ctrl.toggleColumns({columnsToToggle: columnsToToggle.map(getColumnName), visible: visible});
        }
