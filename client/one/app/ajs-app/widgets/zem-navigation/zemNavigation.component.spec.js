describe('ZemNavigationCtrl', function() {
    ...
    it('should provide correct class based on type', function() {
        ...
        expect(ctrl.getItemClasses(item)).toEqual([
            'zem-navigation__item--active',
            'zem-navigation__item--campaign',
        ]);

        ctrl.selectedEntity = item;
        expect(ctrl.getItemClasses(item)).toEqual([
            'zem-navigation__item--active',
            'zem-navigation__item--selected',
            'zem-navigation__item--campaign',
        ]);
        ...
        expect(ctrl.getItemClasses(item)).toEqual([
            'zem-navigation__item--archived',
            'zem-navigation__item--selected',
            'zem-navigation__item--group',
        ]);
    });
    ...
    it('should provide correct icon based on state', function() {
        ...
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--none'
        );

        item.type = constants.entityType.AD_GROUP;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--active'
        );

        item.data.active = constants.infoboxStatus.STOPPED;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--stopped'
        );

        item.data.active = constants.infoboxStatus.INACTIVE;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--inactive'
        );

        item.data.active = constants.infoboxStatus.AUTOPILOT;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--autopilot'
        );
        ...
        item.data.active =
            constants.infoboxStatus.CAMPAIGNSTOP_PENDING_BUDGET_ACTIVE;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--active'
        );

        item.data.active =
            constants.infoboxStatus.CAMPAIGNSTOP_PENDING_BUDGET_ACTIVE_PRICE_DISCOVERY;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--active'
        );

        item.data.active =
            constants.infoboxStatus.CAMPAIGNSTOP_PENDING_BUDGET_AUTOPILOT;
        expect(ctrl.getItemIconClass(item)).toEqual(
            'zem-navigation__item-icon--autopilot'
        );
    });
