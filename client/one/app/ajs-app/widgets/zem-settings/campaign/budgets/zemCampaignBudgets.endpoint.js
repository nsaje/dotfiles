angular.module('one.services').service('zemCampaignBudgetsEndpoint', function ($http, $q) {
    ...
    function create (campaignId, budget) {
        var url = '/api/campaigns/' + campaignId + '/budget/';
        return $http.put(url, convertDataToApi(budget)).then(processResponse, processError);
    }
