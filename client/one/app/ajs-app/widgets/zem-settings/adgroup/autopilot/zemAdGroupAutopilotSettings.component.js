...
angular.module('one.widgets').component('zemAdGroupAutopilotSettings', {
    ...
    controller: function($q, $state, config, zemNavigationNewService) {
        ...
        $ctrl.$onChanges = function() {
            ...
            $ctrl.origAutopilotSettings = {
                b1SourcesGroupEnabled:
                    $ctrl.entity.settings.b1SourcesGroupEnabled,
            };

            $ctrl.b1SourcesGroupEnabled =
                $ctrl.entity.settings.b1SourcesGroupEnabled;
        ...
        function getBudgetAutopilotOptimizationGoalText() {
            ...
            options.budgetAutomationGoals.forEach(function(goal) {
                if (
                    goal.value ===
                    $ctrl.entity.settings.autopilotOptimizationGoal
                ) {
                    goalName = goal.name;
                }
            });
        ...
        // prettier-ignore
        function validate(updateData) { // eslint-disable-line complexity
            if (
                $ctrl.origAutopilotSettings.b1SourcesGroupEnabled ===
                updateData.settings.b1SourcesGroupEnabled
            ) {
                return $q.resolve();
            }
