...
angular
    .module('one.widgets')
    .directive('zemRetargeting', function(config, $state) {
        return {
            restrict: 'E',
            scope: {
            ...
            controller: function($scope) {
                ...
                $scope.selectedAdgroups = function() {
                    ...
                    $scope.retargetableAdgroups.forEach(function(adgroup) {
                        if (
                            $scope.selectedAdgroupIds.indexOf(adgroup.id) >= 0
                        ) {
                            result.push(adgroup);
                        }
                    });
                ...
                $scope.availableAdgroups = function() {
                    ...
                    var adgroups = $scope.retargetableAdgroups
                        .filter(function(adgroup) {
                            return (
                                $scope.selectedAdgroupIds.indexOf(adgroup.id) <
                                0
                            );
                        })
                        .filter(function(adgroup) {
                ...
                $scope.removeSelectedAdgroup = function(adgroup) {
                    var selectedIdx = $scope.selectedAdgroupIds.indexOf(
                        adgroup.id
                    );
                    if (selectedIdx >= 0) {
