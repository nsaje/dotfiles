angular.module('one.widgets').service('zemAccountCreditEndpoint', function ($q, $http, zemUtils) {
    this.list = list;
    this.create = create;
    this.update = update;
    this.get = get;
    ...
    function list (accountId) {
        ...
        $http.get(url)
            .then(function (data) {
                deferred.resolve(convertCreditListFromApi(zemUtils.convertToCamelCase(data.data.data)));
            })
    ...
    function create (accountId, item) {
        ...
        var deferred = $q.defer();
        $http.put(url, zemUtils.convertToUnderscore(convertCreditItemToApi(item)))
            .then(function (data) {
                deferred.resolve(data.data.data);
            })
            .catch(function (error) {
                deferred.reject(zemUtils.convertToCamelCase(convertErrorsFromApi(error.data.data.errors)));
            });
    ...
    function update (accountId, item) {
        ...
        var deferred = $q.defer();
        $http.post(url, zemUtils.convertToUnderscore(convertCreditItemToApi(item)))
            .then(function (data) {
                deferred.resolve(data.data.data);
            })
            .catch(function (error) {
    ...
    function get (accountId, itemId) {
        ...
        $http.get(url)
            .then(function (data) {
                deferred.resolve(convertCreditItemFromApi(zemUtils.convertToCamelCase(data.data.data)));
            })
            .catch(function (error) {
                deferred.reject(error);
            });
