angular.module('one.widgets').service('zemAccountCreditStateService', function (zemAccountCreditEndpoint, zemAccountCreditRefundEndpoint, zemUserService, $q, $filter) { // eslint-disable-line max-len
    ...
    function zemAccountCreditStateService (account) {
        ...
        function reloadCreditRefunds () {
            ...
            return zemAccountCreditRefundEndpoint.list(account.id)
                .then(function (data) {
        ...
        function saveCreditRefundItem () {
            ...
            return action(account.id, state.creditItem.id, state.creditRefundItem)
        ...
        function calculateCreditRefundTotals () {
            ...
            var credits = state.credit.active.concat(state.credit.past).reduce(function (creditMap, credit) {
                creditMap[credit.id] = credit;
                return creditMap;
            }, {});
            Object.keys(refunds).forEach(function (creditId) {
                totals[creditId] = refunds[creditId].reduce(function (total, refund) {
