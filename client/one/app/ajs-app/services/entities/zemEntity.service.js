angular.module('one.services').service('zemEntityService', function ($http, $q, zemAccountService, zemCampaignService, zemAdGroupService, zemContentAdService) { // eslint-disable-line max-len
    ...
    function getEntityService (entityType) {
        if (entityType === constants.entityType.ACCOUNT) return zemAccountService;
        if (entityType === constants.entityType.CAMPAIGN) return zemCampaignService;
        if (entityType === constants.entityType.AD_GROUP) return zemAdGroupService;
        if (entityType === constants.entityType.CONTENT_AD) return zemContentAdService;
    }
