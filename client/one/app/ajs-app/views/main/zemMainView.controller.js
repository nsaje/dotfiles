angular.module('one.views').controller('zemMainView', function ($scope, $location, $q, $state, zemNavigationService) {
    ...
    function checkStateChange () {
        ...
        checksPromise = $q.all([canActivateMainState(), checkRedirectToDefaultAccount(), checkIfEntityExists()])
            .then(function () {
                checksPromise = null;
    ...
    function checkIfEntityExists () {
        ...
        entityGetter($state.params.id)
            ...
            .catch(function () {
                deferred.reject({
                    state: 'v2.analytics',
                    params: {level: constants.levelStateParam.ACCOUNTS, id: null},
                });
            });
        return deferred.promise;
    }
    ...
    function getEntityGetter (level) {
        switch (level) {
        case constants.levelStateParam.ACCOUNT: return zemNavigationService.getAccount;
        case constants.levelStateParam.CAMPAIGN: return zemNavigationService.getCampaign;
        case constants.levelStateParam.AD_GROUP: return zemNavigationService.getAdGroup;
        }
    }
});
