angular.module('one.services').service('zemEntityBulkActionsEndpoint', function ($http, $q) { // eslint-disable-line max-len
    ...
    function archive (level, breakdown, parentId, selection) {
        return post(getUrl(level, breakdown, parentId, 'archive'), createSelectionData(selection));
    }

    function restore (level, breakdown, parentId, selection) {
        return post(getUrl(level, breakdown, parentId, 'restore'), createSelectionData(selection));
    }
