angular.module('one.services').service('zemEntityActionsEndpoint', function ($http, $q) { // eslint-disable-line max-len
    ...
    var mapActionsUrls = {};
    mapActionsUrls[constants.entityType.ACCOUNT] = '/api/accounts/$(id)/$(action)/';
    mapActionsUrls[constants.entityType.CAMPAIGN] = '/api/campaigns/$(id)/$(action)/';
    mapActionsUrls[constants.entityType.AD_GROUP] = '/api/ad_groups/$(id)/$(action)/';

    function getActionUrl (entityType, id, action) {
        if (action === ACTION_ACTIVATE || action === ACTION_DEACTIVATE) {
            return mapActionsUrls[entityType].replace('$(id)', id).replace('$(action)', 'settings/state');
        }
