describe('zemEntityBulkActionsEndpoint', function () {
    ...
    it('should call correct Entity ACTION URLs', function () {
        zemEntityBulkActionsEndpoint.activate(constants.level.ALL_ACCOUNTS, constants.breakdown.ACCOUNT, undefined, {});
    ...
    it('should call correct Source ACTION URLs', function () {
        zemEntityBulkActionsEndpoint.activate(constants.level.ACCOUNTS, constants.breakdown.MEDIA_SOURCE, 1, {});
        zemEntityBulkActionsEndpoint.deactivate(constants.level.CAMPAIGNS, constants.breakdown.MEDIA_SOURCE, 2, {});
        zemEntityBulkActionsEndpoint.activate(constants.level.AD_GROUPS, constants.breakdown.MEDIA_SOURCE, 3, {});

        $httpBackend.expectPOST('/api/accounts/1/sources/state/', {state: constants.settingsState.ACTIVE});
    ...
    it('should post correct selection data', function () {
        ...
        zemEntityBulkActionsEndpoint.archive(constants.level.ACCOUNTS, constants.breakdown.MEDIA_SOURCE, 1, selection);
    ...
    it('should post correct state data', function () {
        var selection = {};
        zemEntityBulkActionsEndpoint.activate(constants.level.ACCOUNTS, constants.breakdown.MEDIA_SOURCE, 1, selection);
        zemEntityBulkActionsEndpoint.deactivate(constants.level.CAMPAIGNS, constants.breakdown.AD_GROUP, 1, selection);

        $httpBackend.expectPOST('/api/accounts/1/sources/state/', {state: constants.settingsState.ACTIVE});
        $httpBackend.expectPOST('/api/campaigns/1/ad_groups/state/', {state: constants.settingsState.INACTIVE});
        $httpBackend.flush();
    });
});
