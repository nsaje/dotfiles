angular.module('one.services').service('zemEntityActionsService', function ($http, $q, zemEntityActionsEndpoint, zemEntityBulkActionsEndpoint, zemPubSubService) { // eslint-disable-line max-len
    ...
    function EntityActionsService (entityType) {
        ...
        // Entity actions
        this.activate = createExecuteAction(constants.entityAction.ACTIVATE);
        ...
        this.deactivateEntities = createBulkExecuteAction(constants.entityAction.DEACTIVATE, parentLevel, breakdown);
        ...
        this.activateSources =
            createBulkExecuteAction(constants.entityAction.ACTIVATE, level, constants.breakdown.MEDIA_SOURCE);
        this.deactivateSources =
            createBulkExecuteAction(constants.entityAction.DEACTIVATE, level, constants.breakdown.MEDIA_SOURCE);
        ...
        function getAction (actionType, action, breakdown) {
            if (actionType === constants.entityActionType.SINGLE) return mapEntityActions[action];
            if (actionType === constants.entityActionType.BULK) {
                ...
                } else if (breakdown === constants.breakdown.PUBLISHER) return mapPublisherBulkActions[action];
        ...
        function createBulkExecuteAction (action, level, breakdown) {
            var actionFn = function (id, selection) {
                ...
                return fn(level, breakdown, id, selection).then(function (data) {
                    pubsub.notify(EVENTS.ON_ENTITY_ACTION_EXECUTED, {
                        action: action,
        ...
        function getParentLevel (entityType) {
            if (entityType === constants.entityType.ACCOUNT) return constants.level.ALL_ACCOUNTS;
            if (entityType === constants.entityType.CAMPAIGN) return constants.level.ACCOUNTS;
            if (entityType === constants.entityType.AD_GROUP) return constants.level.CAMPAIGNS;
            if (entityType === constants.entityType.CONTENT_AD) return constants.level.AD_GROUPS;
            throw Error('Unexpected entity type: ' + entityType);
        }

        function getLevel (entityType) {
            if (entityType === constants.entityType.ACCOUNT) return constants.level.ACCOUNTS;
            if (entityType === constants.entityType.CAMPAIGN) return constants.level.CAMPAIGNS;
            if (entityType === constants.entityType.AD_GROUP) return constants.level.AD_GROUPS;
            if (entityType === constants.entityType.CONTENT_AD) return null;
            throw Error('Unexpected entity type: ' + entityType);
        }

        function getBreakdown (entityType) {
            if (entityType === constants.entityType.ACCOUNT) return constants.breakdown.ACCOUNT;
            if (entityType === constants.entityType.CAMPAIGN) return constants.breakdown.CAMPAIGN;
            if (entityType === constants.entityType.AD_GROUP) return constants.breakdown.AD_GROUP;
            if (entityType === constants.entityType.CONTENT_AD) return constants.breakdown.CONTENT_AD;
            throw Error('Unexpected entity type: ' + entityType);
        }
        ...
        function onActionExecuted (callback) {
            return pubsub.register(EVENTS.ON_ENTITY_ACTION_EXECUTED, callback);
        }
    }
