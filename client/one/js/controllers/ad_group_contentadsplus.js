...
oneApp.controller('AdGroupAdsPlusCtrl', ['$scope', '$window', '$compile', '$state', '$modal', '$location', 'api', 'zemUserSettings', 'zemCustomTableColsService', '$timeout', 'zemFilterService', function ($scope, $window, $compile, $state, $modal, $location, api, zemUserSettings, zemCustomTableColsService, $timeout, zemFilterService) {
    ...
    $scope.bulkArchiveContentAds = function (contentAdIdsEnabled, contentAdIdsDisabled) {
    ...
    $scope.bulkRestoreContentAds = function (contentAdIdsEnabled, contentAdIdsDisabled) {
    ...
    $scope.updateTableAfterArchiving = function(data) {
        ...
        $scope.updateTableData(data.data.rows, {});
    ...
    var getEnabledAndDisabledContentAds = function() {
        ...
        return {enabled: contentAdIdsEnabled, disabled: contentAdIdsDisabled};
    };

    $scope.executeBulkAction = function () {
        var contentAdSelection = getEnabledAndDisabledContentAds();

        var contentAdIdsEnabled = contentAdSelection.enabled,
            contentAdIdsDisabled = contentAdSelection.disabled;
        ...
        switch ($scope.selectedBulkAction) {
            ...
            case 'download':
                ...
                break;
            case 'archive':
                $scope.bulkArchiveContentAds(contentAdIdsEnabled, contentAdIdsDisabled);
                break;
            case 'restore':
                $scope.bulkRestoreContentAds(contentAdIdsEnabled, contentAdIdsDisabled);
                break;
        }
        ...
        api.adGroupAdsPlusTable.getUpdates($state.params.id, $scope.lastChange)
            .then(function (data) {
                if (data.lastChange) {
                    ...
                    $scope.updateTableData(data.rows, data.totals);
                }
    ...
    $scope.updateTableData = function (rowsUpdates, totalsUpdates) {
        $scope.rows.forEach(function (row) {
