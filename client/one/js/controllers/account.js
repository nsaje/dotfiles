...
oneApp.controller('AccountCtrl', ['$scope', '$state', 'zemNavigationService', 'accountData', function ($scope, $state, zemNavigationService, accountData) {
    ...
    $scope.getTabs = function () {
        return [
            ...
            {heading: 'Settings', route: 'main.accounts.settings', active: false, hidden: !$scope.hasPermission('zemauth.account_account_view') || ($scope.hasPermission('zemauth.view_archived_entities') && $scope.account && $scope.account.archived), internal: $scope.isPermissionInternal('zemauth.account_account_view')},
            ...
            {heading: 'Reports', route: 'main.accounts.scheduled_reports', active: false, hidden: ($scope.hasPermission('zemauth.view_archived_entities') && $scope.account && $scope.account.archived), internal: false},
        ];
    };
