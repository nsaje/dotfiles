...
describe('zemDemoContentAdsService', function() {
    ...
    it('set and apply for content ad', function () {
        expect(JSON.stringify(contentAdsService.apply(1, 1, {}))).toBe('{}');
        expect(JSON.stringify(contentAdsService.apply(1, 1, { test: 1 }))).toBe('{"test":1}');
        contentAdsService.set(1, 1, { test: 2 });
        expect(JSON.stringify(contentAdsService.apply(1, 1, { test: 1 }))).toBe('{"test":2}');
        contentAdsService.set(1, 1, { test1: 1 });
        expect(
            JSON.stringify(contentAdsService.apply(1, 1, { test: 1 }))
        ).toBe('{"test":2,"test1":1}');
        expect(JSON.stringify(contentAdsService.apply(2, 2, {}))).toBe('{}');
    });
