...
var Chart = function () {
    ...
    this.metric1SelectOptions = element.all(by.css('.metric1 .select2-drop ul.select2-results li'));

    this.metric2Select = element(by.css('.select2-container.metric2 .select2-choice'));
    this.metric2SelectOptions = element.all(by.css('.metric2 .select2-drop ul.select2-results li'));
