...
node {
  try {
    ...
    if (env.BRANCH_NAME == 'master') {
      ...
      slackSend channel: "#rnd-z1", color: "#FF0000", failOnError: true, message: "Build Failed - ${committer}: ${commit_message} on ${env.JOB_BASE_NAME}/${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
    }
    throw e
  }
}
