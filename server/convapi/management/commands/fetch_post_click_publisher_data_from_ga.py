...
from reports.models import ContentAdGoalConversionStats, ContentAdPostclickStats
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if ga_date is None:
            ...
            ga_date = datetime.date.fromordinal(datetime.date.today().toordinal() - 1)
        else:
        ...
        for ga_account in ga_accounts:
            ...
            self._update_postclick_stats(ga_date, ga_service, profiles)
            self._update_goal_conversion_stats(ga_date, ga_service, profiles)

    def _delete_old_stats(self, ga_date):
        logger.debug('Deleting old stats: ga_date=%s', ga_date)
        ContentAdPostclickStats.objects.filter(date=ga_date).delete()
        ContentAdGoalConversionStats.objects.filter(date=ga_date).delete()
