...
class TestGAApiReport(TestCase):
    ...
    def test_download(self, _download_stats_from_ga_mock, _get_ga_goals_mock, _get_ga_profiles_mock):
        ...
        self.assertEqual(3, len(ga_reports.entries))
        expected_key = ('2015-12-07', 1, 'yahoo', 'www.test.com')
        self.assertTrue(expected_key in ga_reports.entries)
        expected_key2 = ('2015-12-07', 2, 'yahoo', 'www.foo.com')
        self.assertTrue(expected_key2 in ga_reports.entries)
        self.assertEqual(
            '["/driving/voltbolt/?_z1_adgid=830&_z1_caid=1&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www.test.com", "(not set)", "2", "1", "100.0", "1", "10.0"]',
            ga_reports.entries[expected_key].raw_row)

        # Content ads
        entries = ga_reports.get_content_ad_stats()
        entry = entries[0]
        ...
        self.assertEqual('2015-12-07', entry.report_date)
        ...
        self.assertEqual(2, entry2.content_ad_id)
        self.assertEqual('yahoo', entry2.source_param)
        self.assertEqual('2015-12-07', entry2.report_date)

        self.assertEqual({'E-zin subscription': 1}, entry2.goals)

        # Publishers
        entries = ga_reports.get_publisher_stats()
        entry = entries[0]
        self.assertEqual(3, entry.visits)
        self.assertEqual(1.0, entry.bounce_rate)
        self.assertEqual(2, entry.pageviews)
        self.assertEqual(2, entry.new_visits)
        self.assertEqual(3, entry.bounced_visits)
        self.assertEqual(20, entry.total_time_on_site)
        ...
        self.assertEqual({'E-zin subscription': 3}, entry.goals)

        entry2 = entries[1]
        self.assertEqual(2, entry2.visits)
        self.assertEqual(1.0, entry2.bounce_rate)
        self.assertEqual(1, entry2.pageviews)
        self.assertEqual(1, entry2.new_visits)
        self.assertEqual(2, entry2.bounced_visits)
        self.assertEqual(10, entry2.total_time_on_site)

        self.assertEqual(None, entry2.content_ad_id)
        self.assertEqual(1, entry.ad_group_id)
        self.assertEqual('yahoo', entry2.source_param)
        self.assertEqual('2015-12-07', entry2.report_date)

        self.assertEqual({'E-zin subscription': 2}, entry2.goals)
    ...
    def _download_stats_from_ga_side_effect(self, start_date, profile_id, metrics, start_index):
        if metrics == 'ga:sessions,ga:newUsers,ga:bounceRate,ga:pageviews,ga:timeonsite':
            return {'rows': [
                ['/driving/voltbolt/?_z1_adgid=830&_z1_caid=1&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www.test.com', '(not set)', '2', '1', '100.0', '1', '10.0'],
                ['/driving/voltbolt/?_z1_adgid=830&_z1_caid=1&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www%2Efoo%2ecom', '(not set)', '2', '1', '100.0', '1', '10.0'],
                ['/driving/voltbolt/?_z1_adgid=830&_z1_caid=2&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www.foo.com', '(not set)', '1', '1', '100.0', '1', '10.0']
                ], 'itemsPerPage': 1000, 'totalResults': 3}
        elif metrics == 'ga:goal1Completions':
            return {'rows': [
                ['/driving/voltbolt/?_z1_adgid=830&_z1_caid=1&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www.test.com', '(not set)', '2'],
                ['/driving/voltbolt/?_z1_adgid=830&_z1_caid=1&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www%2Efoo%2ecom', '(not set)', '2'],
                ['/driving/voltbolt/?_z1_adgid=830&_z1_caid=2&_z1_msid=yahoo&_z1_disga=zemgagood&_z1_pub=www.foo.com', '(not set)', '1'],
                ], 'itemsPerPage': 1000, 'totalResults': 3}
        else:
