        ...
        for db_name in settings.STATS_DB_WRITE_REPLICAS_POSTGRES:
            ...
            for mv_class in self._required_views():
                ...
                s3_path = os.path.join(
                    materialize_views.MATERIALIZED_VIEWS_REPLICATION_S3_PREFIX,
                    job_id,
                    mv_class.TABLE_NAME,
                )
                manifest = self._find_manifest(s3_path)
                materialize_views.update_table_from_s3_postgres(db_name, manifest, mv_class.TABLE_NAME, date_from, date_to)
