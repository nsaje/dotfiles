...
class PromotionExportTestCase(TestCase):
    ...
    def test_get_promotion_export(self, mock_get_pubs_stats, mock_get_geo_stats, mock_get_ad_stats):
        ...
        mock_get_geo_stats.return_value = [{"state": "US-NY", "impressions": 100}, {"state": None, "impressions": 23}]
        mock_get_pubs_stats.return_value = [{"publisher": "cnn.com"}]

        expected_response = {
            "response": {
                "article": {"description": "Example description", "title": "Title 1"},
                "statistics": {
        ...
        response = self.client.get(
            reverse("businesswire_promotion_export"), {"article_id": "bizwire_article_1"}, follow=True
        )
