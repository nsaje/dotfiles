...
def get_missing_keys(num_days=FIND_MISSING_NUM_DAYS):
    ...
    keys = [k for k in helpers.get_s3_keys_for_dates(dates) if helpers.get_s3_key_dt(k).date() >= config.START_DATE]
    labels_keys = {helpers.get_s3_key_label(key): key for key in keys}

    content_ad_labels = set(
        dash.models.ContentAd.objects.filter(
            ad_group__campaign_id=config.AUTOMATION_CAMPAIGN, label__in=list(labels_keys.keys())
        ).values_list("label", flat=True)
    )

    to_reprocess = set([l for l in list(labels_keys.keys()) if l not in content_ad_labels])
    candidate_labels = set(
        dash.models.ContentAd.objects.filter(
            ad_group__campaign_id=config.AUTOMATION_CAMPAIGN, label__in=to_reprocess
        ).values_list("label", flat=True)
    )
...
def purge_candidates(keys):
    ...
    candidates = dash.models.ContentAdCandidate.objects.filter(
        ad_group__campaign_id=config.AUTOMATION_CAMPAIGN, label__in=labels
    )
...
def invoke_lambdas(keys):
    ...
    for key in keys:
        ...
        lambda_client.invoke(
            FunctionName="z1-businesswire-articles", InvocationType="Event", Payload=json.dumps(payload)  # async
        )
