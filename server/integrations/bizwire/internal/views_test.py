...
@patch.object(random, "random", MagicMock(return_value=1.0))
...
class ArticleUploadTest(TestCase):
    ...
    def test_article_upload(self):
        ...
        response = self.client.post(
            reverse("businesswire_article_upload"), data=json.dumps(articles_data), content_type="application/json"
        )

        self.assertEqual(200, response.status_code)
        self.assertEqual({"status": "ok"}, json.loads(response.content))

        self.assertEqual(1, dash.models.ContentAdCandidate.objects.filter(ad_group_id=1).count())
        self.assertEqual(
            "Article bizwire_article_2", dash.models.UploadBatch.objects.filter(ad_group_id=1).latest("created_dt").name
        )
        ...
        expected_group_daily_budget = math.ceil(
            num_ads * config.DAILY_BUDGET_PER_ARTICLE * (1 - config.OB_DAILY_BUDGET_PCT)
        )
    ...
    def test_duplicate(self):
        ...
        response = self.client.post(
            reverse("businesswire_article_upload"), data=json.dumps([article_data]), content_type="application/json"
        )
    ...
    def test_multiple(self):
        ...
        self.assertEqual(
            "Multiple articles upload", dash.models.UploadBatch.objects.filter(ad_group_id=1).latest("created_dt").name
        )
...
class ClickCappingTest(TestCase):
    ...
    def test_click_capping(self):
        ...
        response = self.client.get(reverse("businesswire_click_capping"), {"creativeId": 1})
        self.assertEqual(200, response.status_code)
