...
class VideoUploadCallbackTestCase(TestCase):
    ...
    def test_put(self):
        data = {
            "status": "PROCESSING",
            "duration": 23,
            "formats": [{"width": 123, "height": 321, "bitrate": 4500, "mime": "video/mp4", "filename": "x.mp4"}],
        }
        r = self.client.put(
            reverse("service.videoassets", kwargs={"videoasset_id": self.videoasset.id}), data=data, format="json"
        )
