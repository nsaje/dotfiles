...
def get_page_id(page_url, access_token):
    ...
    if response.status_code != http.client.OK:
        logger.error(
            "Error while retrieving facebook page id. Status code: %s, Error %s", response.status_code, response.content
        )
        return None
...
def send_page_access_request(page_id, business_id, access_token):
    ...
    response = requests.post(
        FB_PAGES_URL.format(api_version=FB_API_VERSION, business_id=business_id),
    ...
    if response.status_code == 200:
        ...
        already_connected = (
            ERROR_ALREADY_CONNECTED in error.get("error_user_title", "")
            or ERROR_ALREADY_BELONGS_TO_BUSINESS_MANAGER in error.get("message", "")
            or ERROR_ALREADY_ACCESS in error.get("error_user_msg", "")
        )
        ...
        if already_connected:
            ...
            logger.warning(
                "FB api returned an invalid page error for pageId: %s. Error message: %s", page_id, error.get("message")
            )
            return constants.FacebookPageRequestType.INVALID

    error = response.json().get("error", {})
    logger.error(
        "FB api returned and unknown error for pageId: %s. Status code: %s, error message: %s",
        page_id,
...
def get_all_pages(business_id, access_token):
    ...
    if response.status_code != http.client.OK:
        logger.error(
            "Error while accessing facebook page api. Status code: %s, Error %s", response.status_code, response.content
        )
        return None
...
def create_ad_account(name, page_id, app_id, business_id, access_token):
    ...
    response = requests.post(
        FB_AD_ACCOUNT_CREATE_URL.format(api_version=FB_API_VERSION, business_id=business_id),
        json.dumps(params),
        headers=HEADERS,
    )
    if response.status_code != http.client.OK:
        logger.error(
            "Error while creating facebook ad account. Status code: %s, Error %s",
            response.status_code,
            response.content,
        )
        return None
...
def add_system_user_permissions(connected_object_id, role, business_id, system_user_id, access_token):
    params = {"business": business_id, "user": system_user_id, "role": role, "access_token": access_token}
    response = requests.post(
        FB_USER_PERMISSIONS_URL.format(api_version=FB_API_VERSION, object_id=connected_object_id),
        json.dumps(params),
        headers=HEADERS,
    )
    if response.status_code != http.client.OK:
        logger.error(
            "Error while adding system user to a connected object. Status code: %s, Error %s",
            response.status_code,
            response.content,
        )
        return False
...
def stop_facebook_media_sources(account):
    current_settings = models.AdGroupSourceSettings.objects.filter(
        ...
        ad_group_source__ad_group__campaign__account=account,
    ...
    for cs in current_settings:
        ...
        ad_groups_to_ping.add(new_settings.ad_group_source.ad_group)
...
def get_ad_account_status(ad_account_id, access_token):
    params = {"access_token": access_token, "fields": "account_status"}
    response = requests.get(
        FB_AD_ACCOUNT_GET_URL.format(api_version=FB_API_VERSION, account_id=ad_account_id), params=params
    )
...
def _get_facebook_page_request_type(fb_status):
    if fb_status == constants.FacebookAccountStatus.ACTIVE:
        ...
        constants.FacebookAccountStatus.PENDING_RISK_REVIEW,
