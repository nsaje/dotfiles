...
class Command(ExceptionCommand):
    ...
    def handle(self, *args, **options):
        ...
        for real_account_pk in real_account_pks:
            ...
            real_account = model_to_dump.objects.filter(pk=real_account_pk).prefetch_related(
                ACCOUNT_DUMP_SETTINGS['prefetch_related']
            )
            real_account.users = demo_users
            self._add_object_dependencies(real_account, ACCOUNT_DUMP_SETTINGS['dependents'])
            self._serialize_fully()
        data = serialize('json', [o for o in self.serialize_list if o is not None], indent=4)
