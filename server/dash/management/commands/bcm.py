...
MODELS = (MODEL_CREDITS, MODEL_BUDGETS)
...
def update_credits(object_list, updates):
    ...
    cursor.execute(
        "UPDATE dash_creditlineitem SET {} WHERE id in ({});".format(
            set_sql, ", ".join(str(c.pk) for c in object_list)
        ),
        values,
    )
...
def update_budgets(object_list, updates):
    ...
    set_sql, values = _updates_to_sql(updates)
    cursor.execute(
        "UPDATE dash_budgetlineitem SET {} WHERE id in ({});".format(
            set_sql, ", ".join(str(b.pk) for b in object_list)
        ),
        values,
    )
...
class Command(BaseCommand):
    ...
    def _validate(self, model, object_list, options):
        ...
        if model == MODEL_CREDITS:
            ...
            self._validate_credit_amounts(set([obj.credit for obj in object_list]))
