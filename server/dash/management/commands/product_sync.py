...
class Command(ExceptionCommand):
    ...
    def _sync_campaign(self, config):
        ...
        pixel = pixels.ConversionPixel.objects.get(
    ...
    def _get_ad_groups(self, campaign):
        ad_groups = (entity.AdGroup.objects.filter(campaign=campaign)
    ...
    def _pause_sources_without_retargeting(self, ad_group):
        ...
        for ad_group_source in ad_group_sources:
            if (ad_group_source.source.can_modify_retargeting_automatically() or
    ...
    def _get_audiences(self, campaign):
        existing_audiences = (audiences.Audience.objects
                              .filter(pixel__account_id=campaign.account_id)
    ...
    def _audience_rule(self, product):
        return {
            ...
            'value': SHOPIFY_RE.search(product['url']).group(),
        }
