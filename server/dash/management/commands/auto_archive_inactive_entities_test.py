...
class AutoArchiveTest(TestCase):
    ...
    def test_active(self, spend_mock):
        ...
        campaign.settings.update_unsafe(
            None, created_dt=self.inactive_since + self.time_delta, archived=False
        )
    ...
    def test_inactive_adgroup(self, spend_mock):
        ...
        self.assertEqual(
            ad_group.settings.state, constants.AdGroupSettingsState.INACTIVE
        )
        self.assertEqual(campaign_count, 0)
    ...
    def test_inactive_campaign(self, spend_mock):
        campaign = magic_mixer.blend(models.Campaign)
        campaign.settings.update_unsafe(
            None, created_dt=self.inactive_since, archived=False
        )
        ad_group = magic_mixer.blend(models.AdGroup, campaign=campaign)
        ad_group.settings.update_unsafe(
            None,
    ...
    def test_mixed_adgroups(self, spend_mock):
        ...
        self.assertEqual(
            ad_group_new.settings.state, constants.AdGroupSettingsState.INACTIVE
        )
        self.assertEqual(
            ad_group_old.settings.state, constants.AdGroupSettingsState.INACTIVE
        )
        self.assertEqual(
            ad_group_active.settings.state, constants.AdGroupSettingsState.ACTIVE
        )
        self.assertEqual(
            ad_group_old_active.settings.state, constants.AdGroupSettingsState.INACTIVE
        )
        self.assertEqual(campaign_count, 0)
