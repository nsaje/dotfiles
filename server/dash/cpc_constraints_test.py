...
class CpcConstraintsTestCase(test.TestCase):
    ...
    def test_create(self):
        ...
        cpc_constraints.create(
            constraint_type=constants.CpcConstraintType.OUTBRAIN_BLACKLIST, min_cpc=Decimal("0.05"), agency_id=1
        )
        self.assertTrue(
            models.CpcConstraint.objects.filter(
                agency_id=1, constraint_type=constants.CpcConstraintType.OUTBRAIN_BLACKLIST, min_cpc=Decimal("0.05")
            )
        )
        cpc_constraints.create(min_cpc=Decimal("0.05"), account_id=1)
        self.assertTrue(
            models.CpcConstraint.objects.filter(
                account_id=1, constraint_type=constants.CpcConstraintType.MANUAL, min_cpc=Decimal("0.05")
            )
        )
        cpc_constraints.create(max_cpc=Decimal("2.55"), campaign_id=1)
        self.assertTrue(
            models.CpcConstraint.objects.filter(
                campaign_id=1, constraint_type=constants.CpcConstraintType.MANUAL, max_cpc=Decimal("2.55")
            )
        )
        ...
        self.assertTrue(
            models.CpcConstraint.objects.filter(
                campaign_id=1, source_id=1, constraint_type=constants.CpcConstraintType.MANUAL, min_cpc=Decimal("0.05")
            )
        )
    ...
    def test_create_settings_validation(self):
        ...
        self.assertEqual(
            err3.exception.messages[0],
            "Invalid source settings on some ad groups. Please contact Customer Success Team.",
        )

    def test_enforce_account_rule_on_settings(self):
        settings = models.AdGroupSource.objects.get(ad_group_id=1, source_id=1).get_current_settings()
        self.assertEqual(settings.cpc_cc, Decimal("0.12"))
        settings = models.AdGroupSource.objects.get(ad_group_id=4, source_id=1).get_current_settings()
        self.assertEqual(settings.cpc_cc, Decimal("0.12"))
    ...
    def test_clear(self):
        ...
        models.CpcConstraint.objects.create(
            constraint_type=constants.CpcConstraintType.MANUAL, min_cpc=Decimal("0.05"), account_id=1
        )
...
class AdjustCpcTestCase(test.TestCase):
    ...
    def test_adjust_cpc_min_only(self):
        cpc_constraints.create(min_cpc=Decimal("0.05"), source_id=1, ad_group_id=1)
        cpc = cpc_constraints.adjust_cpc(
            Decimal("0.01"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
        )
        self.assertEqual(cpc, Decimal("0.05"))

        cpc = cpc_constraints.adjust_cpc(
            Decimal("0.60"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
        )
        self.assertEqual(cpc, Decimal("0.60"))

    def test_adjust_cpc_max_only(self):
        cpc_constraints.create(max_cpc=Decimal("0.55"), source_id=1, ad_group_id=1)
        cpc = cpc_constraints.adjust_cpc(
            Decimal("0.50"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
        )
    ...
    def test_adjust_cpc_min_and_max(self):
        ...
        self.assertEqual(cpc, Decimal("0.05"))
        cpc = cpc_constraints.adjust_cpc(
            Decimal("0.10"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
        )
        self.assertEqual(cpc, Decimal("0.10"))
        cpc = cpc_constraints.adjust_cpc(
            Decimal("0.20"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
        )
    ...
    def test_adjust_cpc_min_and_max_rules(self):
        ...
        source = models.Source.objects.get(pk=1)
        cpc = cpc_constraints.adjust_cpc(Decimal("0.01"), rules=rules[source])
        self.assertEqual(cpc, Decimal("0.05"))
        cpc = cpc_constraints.adjust_cpc(Decimal("0.10"), rules=rules[source])
        self.assertEqual(cpc, Decimal("0.10"))
        cpc = cpc_constraints.adjust_cpc(Decimal("0.20"), rules=rules[source])
        self.assertEqual(cpc, Decimal("0.15"))
...
class ValidateCpcTestCase(test.TestCase):
    ...
    def test_validate_cpc(self):
        ...
        self.assertEqual(
            err1.exception.messages[0],
            "Bid CPC is violating some constraints: " "CPC constraint on source AdBlade with min. CPC $0.05",
        )

        cpc_constraints.validate_cpc(
            Decimal("0.01"), source=models.Source.objects.get(pk=2), ad_group=models.AdGroup.objects.get(pk=1)
        )
    ...
    def test_validate_max_cpc_rules(self):
        ...
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(Decimal("0.01"), rules=rules[source])
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(Decimal("0.2"), rules=rules[source])
        cpc_constraints.validate_cpc(Decimal("0.1"), rules=rules[source])
    ...
    def test_validate_cpc_multiple(self):
        cpc_constraints.create(min_cpc=Decimal("0.05"), source_id=1, ad_group_id=1)
        cpc_constraints.create(min_cpc=Decimal("0.02"), source_id=1, campaign_id=1)
        with self.assertRaises(forms.ValidationError) as err1:
            cpc_constraints.validate_cpc(
                Decimal("0.01"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
            )
    ...
    def test_validate_cpc_agency(self):
        ...
        agency = models.Agency(name="Test Agency")
        agency.save(req)
        account = models.Account.objects.get(pk=1)
        account.agency = agency
        account.save(req)
        cpc_constraints.create(min_cpc=Decimal("0.05"), source_id=1, agency=agency)
        cpc_constraints.create(max_cpc=Decimal("0.50"), source_id=1, account=account)
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(
                Decimal("0.01"), source=models.Source.objects.get(pk=1), account=models.Account.objects.get(pk=1)
            )
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(
                Decimal("0.01"), source=models.Source.objects.get(pk=1), campaign=models.Campaign.objects.get(pk=1)
            )
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(
                Decimal("0.01"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
        ...
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(Decimal("0.01"), source=models.Source.objects.get(pk=1), agency=agency)
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(Decimal("0.51"), source=models.Source.objects.get(pk=1), account=account)
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(
                Decimal("0.51"), source=models.Source.objects.get(pk=1), campaign=models.Campaign.objects.get(pk=1)
            )
        with self.assertRaises(forms.ValidationError):
            cpc_constraints.validate_cpc(
                Decimal("0.51"), source=models.Source.objects.get(pk=1), ad_group=models.AdGroup.objects.get(pk=1)
            )
