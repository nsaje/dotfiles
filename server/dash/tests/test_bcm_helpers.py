...
class ProjectionsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.start_date = self.today - datetime.timedelta(5)
        self.end_date = self.today + datetime.timedelta(4)
        self.c = create_credit(
            account_id=2,
            start_date=self.start_date,
            end_date=self.end_date,
        ...
        create_statement(
            budget=self.b,
            date=self.today - datetime.timedelta(5),
            media_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            data_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            license_fee_nano=25 * dash.models.TO_NANO_MULTIPLIER,
        )
        ...
        create_statement(
            budget=self.b,
            date=self.today - datetime.timedelta(4),
            media_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            data_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            license_fee_nano=25 * dash.models.TO_NANO_MULTIPLIER,
        )
        ...
        create_statement(
            budget=self.b,
            date=self.today - datetime.timedelta(3),
            media_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            data_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            license_fee_nano=25 * dash.models.TO_NANO_MULTIPLIER,
        )
        ...
        create_statement(
            budget=self.b,
            date=self.today - datetime.timedelta(2),
            media_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            data_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            license_fee_nano=25 * dash.models.TO_NANO_MULTIPLIER,
        )
        ...
        create_statement(
            budget=self.b,
            date=self.today - datetime.timedelta(1),
            media_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            data_spend_nano=50 * dash.models.TO_NANO_MULTIPLIER,
            license_fee_nano=25 * dash.models.TO_NANO_MULTIPLIER,
        )
    ...
    def test_get_projections(self):
        accounts = dash.models.Account.objects.filter(pk=2)
        start = self.today - datetime.timedelta(5)
        end = self.today - datetime.timedelta(1)

        self.assertEqual(
            dash.bcm_helpers.get_projections(accounts, start, end),
            {
                'spend_projection': {2: Decimal('500.0000')},
                'credit_projection': {2: Decimal('500.0000')},
            }
        )
        ...
        self.assertEqual(
            dash.bcm_helpers.get_projections(accounts, start, end),
            {
                'spend_projection': {2: Decimal('625.0000')},
                'credit_projection': {2: Decimal('1000.0000')},
            }
        )
