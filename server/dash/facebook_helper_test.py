...
class FacebookPageAccessTest(TestCase):
    ...
    def test_update_unchanged_page(self, mock_post, mock_get, mock_stop):
        ...
        facebook_helper.update_facebook_account(
            fb_account, page_url, credentials["business_id"], credentials["access_token"]
        )
    ...
    def test_update_new_page(self, mock_request, mock_page_id, mock_stop):
        ...
        mock_request.return_value = self._create_response(200, '{"response":"All clear"}')
    ...
    def test_update_clear_page(self, mock_request, mock_page_id, mock_stop):
        ...
        credentials = self.get_credentials()
        facebook_helper.update_facebook_account(
            fb_account, None, credentials["business_id"], credentials["access_token"]
        )
    ...
    def test_send_page_access_invalid_page(self, mock_post):
        mock_post.return_value = self._create_response(
            400, '{"error":{"message": "Code: (#100) Param page_id must be a valid page ID not your fake page id"}}'
        )

        page_id = "invalid_page_id"
        credentials = self.get_credentials()
        status = facebook_helper.send_page_access_request(
            page_id, credentials["business_id"], credentials["access_token"]
        )

        mock_post.assert_called_once_with(
            FB_PAGES_URL.format(api_version=FB_API_VERSION, business_id="fake_business_id"),
            data=json.dumps(self._get_params(page_id)),
    ...
    def test_send_page_access_already_connected(self, mock):
        mock.return_value = self._create_response(
            400, '{"error":{"error_user_title": "Code: (#100) You Already Have Access To This Page"}}'
        )
    ...
    def test_send_page_access_approval_sent_successfully(self, mock):
        mock.return_value = self._create_response(200, '{"dummy":{"dummy": "dummy"}}')
    ...
    def test_send_page_access_unknown_error(self, mock):
        mock.return_value = self._create_response(500, '{"dummy":{"dummy": "dummy"}}')
...
class FacebookPagesTest(TestCase):
    ...
    def test_valid_page_id(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            FB_PAGE_ID_URL.format(api_version=FB_API_VERSION, page_id="page_name"), params=self._get_token_json()
        )
    ...
    def test_invalid_page_id(self, mock_get):
        ...
        page_id = facebook_helper.get_page_id(page_url, credentials["access_token"])
        mock_get.assert_called_once_with(
            FB_PAGE_ID_URL.format(api_version=FB_API_VERSION, page_id="invalid_page"), params=self._get_token_json()
        )
    ...
    def test_get_all_pages(self, mock):
        response = {
            "data": [
                {"id": "first", "access_status": "connected"},
...
class FacebookAccountTest(TestCase):
    ...
    def test_create_ad_account(self, mock):
        ...
        ad_account_id = facebook_helper.create_ad_account(
            name, page_id, credentials["app_id"], credentials["business_id"], credentials["access_token"]
        )
        mock.assert_called_once_with(
            FB_AD_ACCOUNT_CREATE_URL.format(api_version=FB_API_VERSION, business_id="fake_business_id"),
            json.dumps(self._get_account_params(name, page_id)),
            headers=self._get_headers(),
        )
    ...
    def test_add_system_user_to_account(self, mock):
        ...
        account_id = "1000"
        credentials = self.get_credentials()
        result = facebook_helper.add_system_user_permissions(
            account_id, "ADMIN", credentials["business_id"], credentials["system_user_id"], credentials["access_token"]
        )
        mock.assert_called_once_with(
            FB_USER_PERMISSIONS_URL.format(api_version=FB_API_VERSION, object_id=account_id),
            json.dumps(self._get_user_params("ADMIN")),
            headers=self._get_headers(),
        )
    ...
    def test_add_system_user_to_page(self, mock):
        ...
        mock.assert_called_once_with(
            FB_USER_PERMISSIONS_URL.format(api_version=FB_API_VERSION, object_id=page_id),
            json.dumps(self._get_user_params("ADVERTISER")),
            headers=self._get_headers(),
        )
        self.assertTrue(result)
    ...
    def test_get_ad_account_status(self, get_mock):
        ...
        get_mock.assert_called_once_with(
            "https://graph.facebook.com/v2.8/act_10153378392231753",
...
class FacebookStopMediaSourcesTest(TestCase):
    ...
    def test_stop_source_on_account(self, save_mock, k1_update_mock):
        ...
        save_mock.assert_called_once_with(None, state=2, system_user=None, write_history=True)
        k1_update_mock.assert_called_once_with({models.AdGroup.objects.get(pk=100)}, msg="facebook.stop_media_source")
