...
def get_updated_ad_group_sources_changes(user, last_change_dt, filtered_sources, ad_group_id_=None):
    ...
    new_last_change_dt, changed_ad_group_sources = helpers.get_ad_group_sources_last_change_dt(
        ad_group_sources, ad_group_sources_settings, last_change_dt=last_change_dt
    )

    notifications = helpers.get_ad_group_sources_notifications(
        ad_group_sources, ad_group_settings, ad_group_sources_settings
    )
    ...
    if new_last_change_dt is not None:
        settings_map = {s.ad_group_source_id: s for s in ad_group_sources_settings}
        ...
        for ad_group_source in changed_ad_group_sources:
            ...
            if ad_group_source.source.source_type.type == constants.SourceType.B1:
                _update_rtb_source_row(ad_group_settings, ad_group_source, rows, notifications)

        response["rows"] = rows
        daily_budget = _get_daily_budget(user, filtered_sources, ad_group_settings, ad_group_sources_settings)
        response["totals"] = {"daily_budget": daily_budget, "current_daily_budget": daily_budget}
...
def _get_daily_budget(user, filtered_sources, ad_group_settings, ad_group_sources_settings):
    ...
    if ad_group_settings.b1_sources_group_enabled:
        ...
        all_rtb_budget = 0
        if ad_group_settings.b1_sources_group_state == constants.AdGroupSourceSettingsState.ACTIVE:
...
def _get_daily_budget_total(user, filtered_sources, ad_group_sources_settings, include_rtb_sources=True):
    ...
    budgets = [
        s.local_daily_budget_cc
        for s in ad_group_sources_settings
        if s is not None
        and s.local_daily_budget_cc is not None
        and (not filtered_sources or s.ad_group_source.source_id in filtered_sources)
        and s.state == constants.AdGroupSourceSettingsState.ACTIVE
        and (include_rtb_sources or s.ad_group_source.source.source_type.type != constants.SourceType.B1)
    ]
