...
class HistoryHelperTests(TestCase):
    ...
    def test_write_none(self):
        ...
        ad_group.write_history("", user=user)
        self.assertEqual(0, dash.models.History.objects.all().count())

        campaign = ad_group.campaign
        campaign.write_history(None, user=user)
        self.assertEqual(0, dash.models.History.objects.all().count())

        account = campaign.account
        account.write_history("", system_user=dash.constants.SystemUserType.CAMPAIGN_STOP)
        self.assertEqual(0, dash.models.History.objects.all().count())

    def test_write_ad_group(self):
        user = User.objects.get(pk=1)
        ad_group = dash.models.AdGroup.objects.get(pk=1)
        ad_group.write_history("Funky test", user=user)
    ...
    def test_write_campaign(self):
        ...
        campaign.write_history("Funky test", user=user)
    ...
    def test_write_account(self):
        ...
        account.write_history("Funky test", system_user=dash.constants.SystemUserType.CAMPAIGN_STOP)
