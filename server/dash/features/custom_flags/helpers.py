...
def get_all_custom_flags_on_ad_group(ad_group, flags_map=None):
    ...
    return _get_flags_dict_list(flags_map, ad_group, "Ad group") + get_all_custom_flags_on_campaign(
        ad_group.campaign, flags_map
    )
...
def get_all_custom_flags_on_campaign(campaign, flags_map=None):
    if not flags_map:
        flags_map = _get_flags_map()
    return _get_flags_dict_list(flags_map, campaign, "Campaign") + get_all_custom_flags_on_account(
        campaign.account, flags_map
    )
