...
def is_readable(ga_property_id):
    ...
    try:
        refresh_mappings()
        if models.GALinkedAccounts.objects.filter(
            customer_ga_account_id=account_id, has_read_and_analyze=True
        ).exists():
            return True
    except Exception:
...
def _get_service(zem_ga_account_email):
    ...
    management_service = googleapiclient.discovery.build(
        MANAGEMENT_API_NAME,
