...
class GALinkedAccountsTest(TestCase):
    ...
    def test_refresh_mappings(self):
        ...
        self.assertTrue(models.GALinkedAccounts.objects.filter(customer_ga_account_id='111', zem_ga_account_email='a-1@gapps.com', has_read_and_analyze=True).exists())
        self.assertTrue(models.GALinkedAccounts.objects.filter(customer_ga_account_id='222', zem_ga_account_email='a-1@gapps.com', has_read_and_analyze=False).exists())
        self.assertTrue(models.GALinkedAccounts.objects.filter(customer_ga_account_id='333', zem_ga_account_email='a-2@gapps.com', has_read_and_analyze=True).exists())

    @mock.patch.object(service, 'refresh_mappings')
    def test_is_readable_exists(self, mock_refresh_mappings):
        models.GALinkedAccounts.objects.create(customer_ga_account_id='444', zem_ga_account_email='a-1@gapps.com', has_read_and_analyze=True)
    ...
    def test_is_readable_refresh(self, mock_refresh_mappings):
        ...
        mock_refresh_mappings.side_effect = lambda: models.GALinkedAccounts.objects.create(customer_ga_account_id='444', zem_ga_account_email='a-1@gapps.com', has_read_and_analyze=True)
        self.assertTrue(service.is_readable('UA-444-3'))
