...
def send_supply_reports():
    ...
    publisher_stats = {}
    source_stats = _get_source_stats(yesterday)
    ...
    for recipient in recipients:
        ...
        if recipient.publishers_report and recipient.source.pk not in publisher_stats:
            publisher_stats[recipient.source.pk] = _get_publisher_stats(recipient, yesterday)
        ...
        send_supply_report_email(
            recipient.email, yesterday, impressions, cost, recipient.custom_subject, publisher_report=publisher_report
        )


def _get_source_stats(date):
    source_stats = {}
    query = """
    ...
    """.format(
        date_query=helpers.get_local_date_query(date.date())
    )
