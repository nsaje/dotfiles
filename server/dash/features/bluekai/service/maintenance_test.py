...
TEST_TAXONOMY = [
    {
        ...
        'description': ('Oracle audiences consist of the highest quality '
        ...
        'categoryPrice': 1.0,
        'categoryType': 'regular',
        'description': ('Oracle Telecommunication audiences contain people '
        ...
        'partner': {'id': 0},
        'pathFromRoot': {'ids': [344, 671901, 840141],
                         'names': ['ROOT', 'Oracle', 'Telecommunications']},
        'sortOrder': 9999,
...
class GetUpdatedCategoriesTestCase(TestCase):
    ...
    def test_get_updated_categories(self):
        ...
        self.assertEqual(
            TEST_TAXONOMY[1]['name'], new_category['name'])
        self.assertEqual(
            TEST_TAXONOMY[1]['description'], new_category['description'])
        self.assertEqual(
            TEST_TAXONOMY[1]['stats']['reach'], new_category['reach'])
        self.assertEqual(
            TEST_TAXONOMY[1]['categoryPrice'], new_category['price'])
        ...
        self.assertEqual(
            TEST_TAXONOMY[0]['name'], updated_category['name'])
        self.assertEqual(
            TEST_TAXONOMY[0]['description'],
            updated_category['description'])
        self.assertEqual(
            TEST_TAXONOMY[0]['stats']['reach'], updated_category['reach'])
        self.assertEqual(
            TEST_TAXONOMY[0]['categoryPrice'], updated_category['price'])
...
class RefreshBluekaiCategoriesTestCase(TestCase):
    ...
    @patch('dash.features.bluekai.models.BlueKaiCategory.objects.create',
           autospec=True)
    @patch('dash.features.bluekai.service.maintenance._get_existing_categories',
           autospec=True)
    @patch('dash.features.bluekai.service.bluekaiapi.get_taxonomy',
           autospec=True)
    def test_refresh_bluekai_categories(
            self, mock_get_taxonomy, mock_get_existing, mock_category_create,
