...
def get_adgroup_infobox_spend(user, ad_group, filtered_sources):
    ...
    try:
        ...
        total_today_spend = lc_helper.default_currency(sum(stat['spend'] for stat in stats))
    except:
        stats = []
    ...
    today_spend = infobox_helpers.OverviewSetting(
        ...
        warning='''Today's spend is calculated in real-time and represents the amount of money spent on media and data costs today. Fees are not included.
The final amount might be different due to post-processing in billing.
Outbrain, Yahoo and Facebook today's spend is delayed by 15 minutes.''',
    )
...
def _format_source_spend(source_stat):
    return '{name}: {spend}'.format(
        ...
        spend=lc_helper.default_currency(source_stat['spend']),
    )
