...
class TestGetAnyAppliedFilters(TestCase):
    def setUp(self):
        self.source1 = magic_mixer.blend(
            core.source.Source,
            id=1,
            content_ad_submission_policy=dash.constants.SourceSubmissionPolicy.AUTOMATIC
        )
        self.source2 = magic_mixer.blend(
            core.source.Source,
            id=2,
            content_ad_submission_policy=dash.constants.SourceSubmissionPolicy.MANUAL
        )
    ...
    def test_lookup(self):
        ...
        self.assertEqual(
            service._get_any_applied_filters(dict(agency=agency1)),
            {(1, 'agency', 1): sf1, (2, 'agency', 1): sf2},
        )
        self.assertEqual(
            service._get_any_applied_filters(dict(agency=agency2)),
            {(2, 'agency', 2): sf3}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            service._get_any_applied_filters(dict(source=magic_mixer.blend(core.source.Source))),
            {}
        )
    ...
    def test_lookup_multiple_levels(self):
        ...
        self.assertEqual(
            service._get_any_applied_filters(dict(campaign__in=[campaign1], account__in=[account1, account2],
                                                  ad_group_id__in=[ad_group1.pk])),
            {
                (2, 'account', 1): sf2,
                (2, 'account', 2): sf3,
                (1, 'campaign', 1): sf4,
        ...
        self.assertEqual(
            service._get_any_applied_filters(dict(campaign__in=[campaign1], agency__in=[
                agency1], content_ad__in=[content_ad1])),
            {
                (1, 'agency', 1): sf1,
                (1, 'campaign', 1): sf4,
                (1, 'content_ad', 1): sf5,
...
class TestFilterValidContentAdSources(TestCase):
    ...
    def _create_content_ad_source_dict(self, content_ad_id, ad_group_id, campaign_id, account_id, agency_id, source_id, policy):
        return {
    ...
    def test_allowed_account(self, get_any):
        ...
        content_ad_sources = [
            self._create_content_ad_source_dict(1, 10, 100, 1000, 10000, 1,
                                                dash.constants.SourceSubmissionPolicy.AUTOMATIC),
            self._create_content_ad_source_dict(1, 10, 100, 1000, 10000, 2,
                                                dash.constants.SourceSubmissionPolicy.MANUAL),
        ]
        self.assertEqual(
            service.filter_valid_content_ad_sources(content_ad_sources),
            [
