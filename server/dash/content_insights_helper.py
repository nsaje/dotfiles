...
def fetch_campaign_content_ad_metrics(user, campaign, start_date, end_date):
    ...
    query_results = redshiftapi.api_breakdowns.query(
        ...
        constraints={"date__gte": start_date, "date__lte": end_date, "campaign_id": campaign.id},
        parents=None,
        goals=goals,
...
def _extract_ends(deduplicated_ads, stats):
    ...
    dd_cad_metric = []
    for title, caids in deduplicated_ads.items():
        dd_cad_metric.append(_extract_ctr_metric(title, caids, mapped_stats))

    active_metrics = [cad_metric for cad_metric in dd_cad_metric if cad_metric["clicks"] >= MIN_BEST_PERFORMER_CLICKS]

    top_cads = sorted(active_metrics, key=lambda dd_cad: dd_cad["value"], reverse=True)[
        :CONTENT_INSIGHTS_TABLE_ROW_COUNT
    ]

    active_metrics = [cad_metric for cad_metric in dd_cad_metric if cad_metric["clicks"] >= MIN_WORST_PERFORMER_CLICKS]

    bott_cads = sorted(active_metrics, key=lambda dd_cad: dd_cad["value"])[:CONTENT_INSIGHTS_TABLE_ROW_COUNT]
    return (
        [{"summary": cad["summary"], "metric": cad["metric"]} for cad in top_cads],
        [{"summary": cad["summary"], "metric": cad["metric"]} for cad in bott_cads],
    )


def _extract_ctr_metric(title, caids, mapped_stats):
    clicks = sum([mapped_stats.get(caid, {}).get("clicks", 0) or 0 for caid in caids])
    impressions = sum([mapped_stats.get(caid, {}).get("impressions", 0) or 0 for caid in caids])
