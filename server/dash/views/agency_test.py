...
from core.models.source_type.instance import SourceTypeMixin
...
class AdGroupSettingsTest(TestCase):
            ...
            "settings": {
                ...
                "bluekai_targeting": {
                    "AND": [
                        ...
                        {"OR": [{"category": "lotame:123"}, {"category": "outbrain:321"}]},
                    ]
                },
            ...
            json_blob,
            {
                "data": {
                    ...
                    "retargetable_adgroups": [
                        {"campaign_name": "test campaign 1", "archived": False, "id": 1, "name": "test adgroup 1 Čžš"},
                        {"campaign_name": "test campaign 2", "archived": False, "id": 2, "name": "test adgroup 2"},
                        {"campaign_name": "test campaign 1", "archived": False, "id": 9, "name": "test adgroup 9"},
                        {"campaign_name": "test campaign 1", "archived": False, "id": 10, "name": "test adgroup 10"},
                        {"campaign_name": "test campaign 1", "archived": False, "id": 987, "name": "test adgroup 1"},
                    ],
                    "audiences": [
                        {"id": 1, "name": "test audience 1", "archived": False},
                        {"id": 2, "name": "test audience 2", "archived": False},
                        {"id": 3, "name": "test audience 3", "archived": True},
                        {"id": 4, "name": "test audience 4", "archived": False},
    ...
    def test_get_local_cpc(self, mock_get_exchange_rate):
        ...
        ad_group.settings.update(None, cpc=Decimal(5))
    ...
    def test_get_local_cpm(self, mock_get_exchange_rate):
        ...
        account = magic_mixer.blend(models.Account, users=[self.user], currency=constants.Currency.EUR)
        campaign = magic_mixer.blend(models.Campaign, account=account)
        ad_group = magic_mixer.blend(models.AdGroup, campaign=campaign, bidding_type=constants.BiddingType.CPM)
        ad_group.settings.update(None, cpm=Decimal(5))
    ...
    def test_put_cpc(self, mock_k1_ping, mock_redirector_insert_adgroup):
        with patch("utils.dates_helper.local_today") as mock_now:
            ...
            ad_group = models.AdGroup.objects.get(pk=1)
            ad_group.settings.update_unsafe(None, cpm=1.600, local_cpm=1.600)

            self.settings_dict["settings"]["b1_sources_group_cpm"] = None
    ...
    @patch.object(SourceTypeMixin, "get_min_cpm", return_value=Decimal("0.10"))
    @patch("utils.redirector_helper.insert_adgroup")
    @patch("utils.k1_helper.update_ad_group")
    def test_put_cpm(self, mock_k1_ping, mock_redirector_insert_adgroup, mock_min_cpm):
        with patch("utils.dates_helper.local_today") as mock_now:
            # mock datetime so that budget is always valid
            mock_now.return_value = datetime.date(2016, 1, 5)
            ...
            ad_group.save(None)
            ad_group.settings.update_unsafe(None, cpc=0.300, local_cpc=0.300)

            self.settings_dict["settings"]["bidding_type"] = constants.BiddingType.CPM
            self.settings_dict["settings"]["cpc_cc"] = None
            self.settings_dict["settings"]["max_cpm"] = "1.600"
            self.settings_dict["settings"]["b1_sources_group_cpc_cc"] = None
            self.settings_dict["settings"]["b1_sources_group_cpm"] = "1.02"
            ...
            mock_redirector_insert_adgroup.assert_has_calls([call(ad_group)])
    ...
    def test_put_local(self, mock_ad_group_settings_update, mock_get_exchange_rate):
        ...
        self.assertIsNone(kwargs.get("cpc"))
        self.assertIsNone(kwargs.get("max"))
        self.assertEqual(kwargs.get("local_cpc"), Decimal("0.5"))
        self.assertEqual(kwargs.get("local_cpm"), Decimal("1.5"))
    ...
    def test_put_low_cpc(self, mock_k1_ping, mock_insert_adgroup):
        with patch("utils.dates_helper.local_today") as mock_now:
            ...
            old_settings = ad_group.get_current_settings()
            self.assertIsNotNone(old_settings.pk)
            self.assertEqual(old_settings.cpc, Decimal("1.000"))
            self.assertEqual(
                set(
    ...
    @patch.object(SourceTypeMixin, "get_min_cpm", return_value=Decimal("0.01"))
    @patch("utils.redirector_helper.insert_adgroup")
    @patch("utils.k1_helper.update_ad_group")
    def test_put_low_cpm(self, mock_k1_ping, mock_insert_adgroup, mock_min_cpm):
        with patch("utils.dates_helper.local_today") as mock_now:
            # mock datetime so that budget is always valid
            mock_now.return_value = datetime.date(2016, 1, 5)

            # fix initial source cpm values
            ad_group = models.AdGroup.objects.get(pk=1)
            for ags in ad_group.adgroupsource_set.all():
                ags.settings.update_unsafe(None, cpm=Decimal("1.000"))
            ...
            old_settings = ad_group.get_current_settings()
            self.assertIsNotNone(old_settings.pk)
            self.assertEqual(old_settings.cpm, Decimal("1.000"))
            self.assertEqual(
                set(
                    ad_group.bidmodifier_set.filter(type=bid_modifiers.BidModifierType.SOURCE).values_list(
                        "modifier", flat=True
    ...
    def test_put_without_bid_values(self):
        ...
        self.assertTrue(
            "CPC value is required for CPC bidding type" in json.loads(response.content)["data"]["errors"]["__all__"]
        )

    @patch("utils.redirector_helper.insert_adgroup")
    @patch("automation.autopilot.recalculate_budgets_ad_group")
    def test_put_set_budget_autopilot_triggers_budget_reallocation_cpc(
    ...
    @patch("utils.redirector_helper.insert_adgroup")
    @patch("automation.autopilot.recalculate_budgets_ad_group")
    def test_put_set_budget_autopilot_triggers_budget_reallocation_cpm(
        self, mock_redirector_insert_adgroup, mock_init_autopilot
    ):
        with patch("utils.dates_helper.local_today") as mock_now:
            # mock datetime so that budget is always valid
            ...
            del self.settings_dict["settings"]["cpc_cc"]
            self.settings_dict["settings"]["max_cpm"] = "0.16"
            self.settings_dict["settings"]["b1_sources_group_cpc_cc"] = str(new_settings.local_b1_sources_group_cpc_cc)
            self.settings_dict["settings"]["b1_sources_group_cpm"] = str(new_settings.local_b1_sources_group_cpm)
            self.settings_dict["settings"]["b1_sources_group_daily_budget"] = str(
                new_settings.local_b1_sources_group_daily_budget
    ...
    def test_rtb_sources_cpc_change_changes_all_rtb_cpcs(self, mock_k1_ping, mock_insert_adgroup):
        with patch("utils.dates_helper.local_today") as mock_now:
            ...
            ad_group = models.AdGroup.objects.get(pk=1)
            ad_group.settings.update_unsafe(None, cpc=7.00, local_cpc=7.00)
            old_settings = ad_group.get_current_settings()
            self.assertIsNotNone(old_settings.pk)

            add_permissions(self.user, ["settings_view", "can_set_rtb_sources_as_one_cpc"])
            self.settings_dict["settings"]["cpc_cc"] = ad_group.settings.cpc
            new_settings = {}
            new_settings.update(self.settings_dict)
            new_settings["settings"]["b1_sources_group_cpc_cc"] = "0.1"
            ...
            mock_k1_ping.assert_has_calls([call(ad_group, msg="AdGroupSettings.put", priority=ANY)])
    ...
    def test_rtb_sources_cpm_change_changes_all_rtb_cpms(self, mock_k1_ping, mock_insert_adgroup):
        with patch("utils.dates_helper.local_today") as mock_now:
            ...
            ad_group.bidding_type = constants.BiddingType.CPM
            ad_group.save(None)
            ad_group.settings.update_unsafe(None, cpm=2.00, local_cpm=2.00)

            del self.settings_dict["settings"]["bidding_type"]
            del self.settings_dict["settings"]["b1_sources_group_cpc_cc"]
            self.settings_dict["settings"]["cpc_cc"] = None
            self.settings_dict["settings"]["max_cpm"] = ad_group.settings.cpm
    ...
    def test_full_autopilot_to_cpc_autopilot_transition(self, mock_k1_ping, mock_insert_adgroup):
        with patch("utils.dates_helper.local_today") as mock_now:
            ...
            add_permissions(
                self.user, ["settings_view", "can_set_adgroup_to_auto_pilot", "can_set_rtb_sources_as_one_cpc"]
            )
            new_settings = {}
            new_settings.update(self.settings_dict)
    ...
    def test_full_autopilot_to_cpm_autopilot_transition(self, mock_k1_ping, mock_insert_adgroup):
        with patch("utils.dates_helper.local_today") as mock_now:
            ...
            mock_insert_adgroup.assert_has_calls([call(ad_group)])
    ...
    def test_deactivate_no_validation_cpc(self):
        ...
        ad_group.settings.update_unsafe(None, cpc=Decimal(5))
        response = self.client.put(
            reverse("ad_group_settings", kwargs={"ad_group_id": ad_group.id}),
            json.dumps(self.settings_dict),
            follow=True,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(ad_group.get_current_settings().cpc, Decimal(5))
    ...
    def test_deactivate_no_validation_cpm(self):
        ...
        ad_group.settings.update_unsafe(None, cpm=Decimal(5))
        response = self.client.put(
            reverse("ad_group_settings", kwargs={"ad_group_id": ad_group.id}),
            json.dumps(self.settings_dict),
            follow=True,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(ad_group.get_current_settings().cpm, Decimal(5))
        self.assertEqual(ad_group.get_current_settings().state, constants.AdGroupSettingsState.INACTIVE)
    ...
    @patch.object(SourceTypeMixin, "get_min_cpm", return_value=Decimal("0.10"))
    def test_put_bidding_type(self, mock_min_cpm):
        ad_group = models.AdGroup.objects.get(pk=1)
        add_permissions(self.user, ["settings_view", "fea_can_use_cpm_buying"])
        ...
        del self.settings_dict["settings"]["cpc_cc"]
        del self.settings_dict["settings"]["b1_sources_group_cpc_cc"]
        del self.settings_dict["settings"]["b1_sources_group_cpm"]
        self.settings_dict["settings"]["max_cpm"] = "1.6000"
        self.settings_dict["settings"]["bidding_type"] = constants.BiddingType.CPM
...
class AdGroupSettingsRetargetableAdgroupsTest(TestCase):
    ...
    def _get_retargetable_adgroups(self, ad_group_id):
        response = self.client.get(reverse("ad_group_settings", kwargs={"ad_group_id": ad_group_id}), follow=True)
...
class CampaignSettingsTest(TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(
            content["data"]["settings"]["target_regions"], self._target_regions_repr(dma=["501"], countries=["NC"])
        )
    ...
    def test_put(
        self,
        mock_k1_ping,
        mock_send_campaign_notification_email,
        mock_send_ga_email,
        mock_r1_insert_adgroup,
        mock_ga_readable,
        mock_autopilot,
    ):
    ...
    def test_put_goals_modified(self, p1, p3, mock_get_exchange_rate):
        ...
        goal = models.CampaignGoal.objects.create_unsafe(
            type=constants.CampaignGoalKPI.CPC, primary=True, campaign_id=1
        )
        models.CampaignGoalValue.objects.create(campaign_goal=goal, value=Decimal("0.1"), local_value=Decimal("0.2"))
            ...
            json.dumps(
                {
                    ...
                    "goals": {"added": [], "removed": [], "modified": {goal.pk: "0.4"}, "primary": goal.pk},
                }
            ),
            content_type="application/json",
        )
        ...
        values = models.CampaignGoalValue.objects.filter(campaign_goal__campaign_id=1).order_by("created_dt")
        self.assertEqual(values[0].value, Decimal("0.1000"))
    ...
    def test_put_goals_removed(self, p1, p3):
        ...
        goal = models.CampaignGoal.objects.create_unsafe(type=1, primary=True, campaign_id=campaign_id)
            ...
            json.dumps(
                {
                    ...
                    "goals": {"added": [], "removed": [{"id": goal.pk}], "modified": {}, "primary": None},
                }
            ),
            content_type="application/json",
        )
    ...
    def test_validation(self):
        ...
        response = self.client.put(
            "/api/campaigns/1/settings/",
            json.dumps({"settings": {"id": 1, "name": "test campaign 2", "campaign_goal": 50}}),
            content_type="application/json",
        )
        content = json.loads(response.content)
        self.assertFalse(content["success"])
    ...
    def test_get_with_conversion_goals(self):
        ...
        models.CampaignGoal.objects.create_unsafe(
            campaign=ad_group.campaign, type=constants.CampaignGoalKPI.CPA, conversion_goal=convg
        )
        ...
        self.assertDictContainsSubset(
            {"name": "janezjanez", "pixel_url": "https://p1.zemanta.com/p/1/2/"},
    ...
    def test_get_campaign_managers_no_permission(self):
        ...
        response = self.client.get("/api/campaigns/1000/settings/")
        content = json.loads(response.content)
    ...
    def test_get_campaign_managers_from_agency(self):
        ...
        self.assertTrue(content["success"])
        self.assertEqual(content["data"].get("campaign_managers"), [{"id": "1", "name": "non_superuser@zemanta.com"}])
...
class AccountSettingsTest(TestCase):
    ...
    def test_get(self):
        ...
        response = client.get(reverse("account_settings", kwargs={"account_id": 1}), follow=True)
    ...
    def test_get_as_agency_manager(self):
        client = self._get_client_with_permissions(["can_modify_account_name", "can_modify_account_manager"])
        user = User.objects.get(pk=2)
        agency = models.Agency.objects.get(pk=1)
        agency.users.add(user)

        response = client.get(reverse("account_settings", kwargs={"account_id": 1000}), follow=True).json()
        self.assertTrue(response["success"])
        self.assertDictEqual(
    ...
    def test_put_as_agency_manager(self):
        ...
        basic_settings = {"id": 1000, "name": "changed name", "default_account_manager": "3", "currency": "USD"}
    ...
    def test_put_as_agency_manager_sources(self):
        ...
        response, _ = self._put_account_agency(client, basic_settings, 1000)
        self.assertEqual(
            response.status_code, 401, msg="Agency manager doesn" "t have permission for changing allowed sources"
        )
    ...
    def test_get_account_manager_users_from_agency(self):
        client = self._get_client_with_permissions(["can_modify_account_manager"])
        user = User.objects.get(pk=2)
        agency = models.Agency.objects.get(pk=1)
        agency.users.add(user)
        ...
        self.assertTrue(response["success"])
        self.assertCountEqual(response["data"]["account_managers"], [{"id": "2", "name": "user@test.com"}])
    ...
    def test_get_account_manager_users_can_see_all_users(self):
        client = self._get_client_with_permissions(["can_modify_account_manager", "can_see_all_users_for_managers"])
        user = User.objects.get(pk=2)
        account = models.Account.objects.get(pk=1000)
        account.users.add(user)
    ...
    def test_get_no_permission_can_modify_account_type(self):
        client = self._get_client_with_permissions([])
        response = client.get(reverse("account_settings", kwargs={"account_id": 1}), follow=True).json()
    ...
    def test_put(self, mock_request, mock_page_id):
        ...
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([1]))

        self.assertDictEqual(
            account_settings.get_settings_dict(),
            {
                "archived": False,
    ...
    def test_put_no_permission_can_modify_account_type(self):
        client = self._get_client_with_permissions(["can_modify_allowed_sources"])
    ...
    def test_get_changes_text_for_media_sources(self):
        ...
        self.assertEqual(view.get_changes_text_for_media_sources([], []), "")
        self.assertEqual(
            view.get_changes_text_for_media_sources(sources[0:1], []), "Added allowed media sources (Source 1)"
        )
        self.assertEqual(
            view.get_changes_text_for_media_sources([], sources[1:2]), "Removed allowed media sources (Source 2)"
        )
    ...
    def test_set_allowed_sources(self):
        ...
        changes_text = view.set_allowed_sources(
            ...
            self._get_form_with_allowed_sources_dict(
                {1: {"allowed": True}, 2: {"allowed": False}, 3: {"allowed": True}}
            ),
        )

        self.assertIsNotNone(changes_text)
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([1, 3]))

    def test_set_allowed_sources_cant_remove_unreleased(self):
        account = models.Account.objects.get(pk=1)
        account.allowed_sources.add(3)  # add an unreleased source
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([1, 2, 3]))
        ...
        changes_text = view.set_allowed_sources(
            ...
            self._get_form_with_allowed_sources_dict(
                {1: {"allowed": False}, 2: {"allowed": False}, 3: {"allowed": False}}
            ),
        )
        self.assertIsNotNone(changes_text)
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([3]))

    def test_set_allowed_sources_cant_add_unreleased(self):
        account = models.Account.objects.get(pk=1)
        account_settings = account.get_current_settings()
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([1, 2]))
        self.assertFalse(models.Source.objects.get(pk=3).released)
        ...
        changes_text = view.set_allowed_sources(
            ...
            self._get_form_with_allowed_sources_dict(
                {1: {"allowed": False}, 2: {"allowed": True}, 3: {"allowed": True}}
            ),
        )
        self.assertIsNotNone(changes_text)
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([2]))

    def test_set_allowed_sources_cant_remove_running_source(self):
        account = models.Account.objects.get(pk=111)
        account_settings = account.get_current_settings()
        self.assertEqual(set(account.allowed_sources.values_list("id", flat=True)), set([2, 3]))
        view = agency.AccountSettings()
        form = self._get_form_with_allowed_sources_dict({2: {"allowed": False}, 3: {"allowed": True}})

        view.set_allowed_sources(account_settings, account, False, form)  # no permission to add unreleased source 3
    ...
    def test_get_allowed_sources(self):
        client = self._get_client_with_permissions(["can_modify_allowed_sources", "can_see_all_available_sources"])

        response = client.get(reverse("account_settings", kwargs={"account_id": 1}), follow=True)
        response = json.loads(response.content)

        self.assertEqual(
            response["data"]["settings"]["allowed_sources"],
            {
    ...
    def test_add_error_to_account_agency_single(self):
        ...
        self.assertEqual(
            dict(form.errors),
            {"allowed_sources": ["Can't save changes because media source Source 1 is still used on this account."]},
        )
    ...
    def test_change_currency_account_has_campaigns(self):
        ...
        response = client.put(
            reverse("account_settings", kwargs={"account_id": 1}),
            json.dumps({"settings": {"id": 1, "currency": "EUR"}}),
            content_type="application/json",
        )
        content = json.loads(response.content)
        self.assertFalse(content["success"])
    ...
    def test_change_currency(self):
        ...
        response = client.put(
            ...
            json.dumps({"settings": {"id": account.id, "currency": "EUR"}}),
            content_type="application/json",
        )
        content = json.loads(response.content)
        self.assertTrue(content["success"])
