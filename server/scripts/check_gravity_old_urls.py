...
CAMPAIGN_GET_ARTICLES_URL = (
    "https://dashboard.gravity.com/api/campaign-article-settings?" "campaignKey={campaign_id}&page=1&sg={site_token}"
)
...
def _login(session):
    ...
    try:
        response = session.post(SIGNIN_URL_USER, data={"email": EMAIL, "password": PASSWORD}, allow_redirects=True)
    except Exception:
        et, ei, tb = sys.exc_info()
        raise Exception(ei).with_traceback(tb)

    # the response is empty, only check status code
    check_response_error(response, Exception, "Login to /user failed.")

    try:
        response = session.post(
            SIGNIN_URL_PUBLIC_LARAVEL, data={"email": EMAIL, "password": PASSWORD}, allow_redirects=True
        )
    except Exception:
        et, ei, tb = sys.exc_info()
        raise Exception(ei).with_traceback(tb)

    check_response_error(response, Exception, "Login to /public failed.")
...
if __name__ == "__main__":
    ...
    if "AD_GROUP_IDS" in globals() and AD_GROUP_IDS:
        ...
        ad_group_ids = [
            ags.ad_group_id for ags in models.AdGroupSource.objects.filter(can_manage_content_ads=True, source_id=2)
        ]
    ...
    for ad_group_id in ad_group_ids:
        ...
        print(
            "AD GROUP ID: {} NUM LINKS: {}, OLD URLS: {}".format(
                ad_group_id, len(articles), len(old_urls_gravity.get(ad_group_id, []))
            )
        )
