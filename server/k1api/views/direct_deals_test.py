...
class DirectDealsTest(K1APIBaseTest):
    ...
    def setUpTestData(cls):
        ...
        magic_mixer.blend(core.direct_deals.DirectDealConnection, source=source, deals=[deal1], adgroup=adgroup,  exclusive=False)
        magic_mixer.blend(core.direct_deals.DirectDealConnection, source=source, deals=[deal2], agency=agency, exclusive=False)
        magic_mixer.blend(core.direct_deals.DirectDealConnection, source=source, deals=[deal1, deal2], adgroup=adgroup2, exclusive=True)

    def test_get_direct_deals(self):
        response = self.client.get(
            reverse('k1api.directdeals'),
        )
