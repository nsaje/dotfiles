...
@mock.patch("automation.campaignstop.get_campaignstop_states")
class AdGroupsSourcesGroupsTest(K1APIBaseTest):
    def setUp(self):
        super().setUp()

        parent_source_1 = magic_mixer.blend(dash.models.Source, id=settings.HARDCODED_SOURCE_ID_OUTBRAINRTB)
        parent_source_2 = magic_mixer.blend(dash.models.Source, id=90)
        grouped_sources = magic_mixer.cycle(5).blend(dash.models.Source, id=(sid for sid in [81, 82, 83, 91, 92]))

        self.ad_group = magic_mixer.blend(dash.models.AdGroup, campaign__account__agency__uses_source_groups=True)
        self.ad_group.settings.update_unsafe(None, state=dash.constants.AdGroupSettingsState.ACTIVE)

        self.main_ad_group_source_1 = magic_mixer.blend(
            dash.models.AdGroupSource,
            ad_group=self.ad_group,
            source=parent_source_1,
            source_campaign_key="source_campaign_key_1",
            blockers=["blocker1"],
        )
        self.main_ad_group_source_1.settings.update_unsafe(
            None, state=dash.constants.AdGroupSourceSettingsState.ACTIVE, daily_budget_cc=10
        )
        self.main_ad_group_source_2 = magic_mixer.blend(
            dash.models.AdGroupSource,
            ad_group=self.ad_group,
            source=parent_source_2,
            source_campaign_key="source_campaign_key_2",
            blockers=["blocker2"],
        )
        self.main_ad_group_source_2.settings.update_unsafe(
            None, state=dash.constants.AdGroupSourceSettingsState.INACTIVE, daily_budget_cc=20
        )

        # deprecated ad group sources
        grouped_ad_group_sources = magic_mixer.cycle(5).blend(
            dash.models.AdGroupSource,
            ad_group=self.ad_group,
            source=(s for s in grouped_sources),
            source_campaign_key=("sck" + str(i) for i in range(5)),
            blockers=(["block" + str(i)] for i in range(5)),
        )
        for ad_group_source in grouped_ad_group_sources:
            ad_group_source.settings.update_unsafe(
                None, state=dash.constants.AdGroupSourceSettingsState.ACTIVE, daily_budget_cc=50
            )

        self.campaignstop_states = collections.defaultdict(
            lambda: {"allowed_to_run": True, "min_allowed_start_date": dates_helper.local_today()}
        )

    def test_get_grouped_ad_groups_sources(self, mock_get_campaignstop_states):
        mock_get_campaignstop_states.return_value = self.campaignstop_states

        response = self.client.get(reverse("k1api.ad_groups.sources"), {"ad_group_ids": self.ad_group.id})
        ...
        for entry in data:
            ...
            if source_id in [settings.HARDCODED_SOURCE_ID_OUTBRAINRTB, 81, 82, 83]:
                target_ad_group_source = self.main_ad_group_source_1
            if source_id in [90, 91, 92]:
                target_ad_group_source = self.main_ad_group_source_2

            if source_id in [settings.HARDCODED_SOURCE_ID_OUTBRAINRTB, 90]:
                self.assertEqual(target_ad_group_source.source_id, entry["source_id"])
                self.assertEqual(target_ad_group_source.source.bidder_slug, entry["slug"])
            else:
                self.assertNotEqual(target_ad_group_source.source_id, entry["source_id"])
                self.assertNotEqual(target_ad_group_source.source.bidder_slug, entry["slug"])

            self.assertEqual(target_ad_group_source.ad_group_id, entry["ad_group_id"])
            self.assertEqual(target_ad_group_source.settings.state, entry["state"])
            self.assertEqual(target_ad_group_source.source_campaign_key, entry["source_campaign_key"])
            self.assertEqual(target_ad_group_source.blockers, entry["blockers"])

    def test_get_grouped_ad_groups_sources_no_flag(self, mock_get_campaignstop_states):
        mock_get_campaignstop_states.return_value = self.campaignstop_states
        ...
        for entry in data:
            ...
            if source_id in [settings.HARDCODED_SOURCE_ID_OUTBRAINRTB, 90]:
                ...
                self.assertNotEqual(target_ad_group_source.source_id, entry["source_id"])
                self.assertNotEqual(target_ad_group_source.source.bidder_slug, entry["slug"])
                self.assertNotEqual(target_ad_group_source.source_campaign_key, entry["source_campaign_key"])
                self.assertNotEqual(target_ad_group_source.blockers, entry["blockers"])
                self.assertEqual(dash.constants.AdGroupSourceSettingsState.ACTIVE, entry["state"])

            self.assertEqual(target_ad_group_source.ad_group_id, entry["ad_group_id"])

    def test_get_grouped_ad_groups_sources_oen(self, mock_get_campaignstop_states):
