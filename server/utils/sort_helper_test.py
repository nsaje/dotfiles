...
class SortHelperTestCase(unittest.TestCase):
    ...
    def test_sort_results_single_field(self):
        results = [{"name": "bbb"}, {"name": "ccc"}, {"name": "aaa"}, {"name": None}]

        sorted_results = sort_helper.sort_results(results, ["name"])
        self.assertEqual([result["name"] for result in sorted_results], ["aaa", "bbb", "ccc", None])

        sorted_results = sort_helper.sort_results(results, ["-name"])
        self.assertEqual([result["name"] for result in sorted_results], ["ccc", "bbb", "aaa", None])
...
class MapByBreakdownTestCase(unittest.TestCase):
    def test_map_single_level(self):
        ...
        self.assertCountEqual(
            sort_helper.map_by_breakdown(rows, ["source"], lambda row: row["value"]), {1: 55, 2: 44, 3: 33, 4: 22}
        )

    def test_map_multilevel(self):
        rows = [
            {"source": 1, "ad_group": 2, "day_index": 3, "value": 55},
            {"source": 2, "ad_group": 2, "day_index": 3, "value": 44},
            {"source": 1, "ad_group": 3, "day_index": 3, "value": 33},
            {"source": 1, "ad_group": 2, "day_index": 5, "value": 22},
        ]

        self.assertCountEqual(
            sort_helper.map_by_breakdown(rows, ["source", "ad_group", "day_index"], lambda row: row["value"]),
            {1: {2: {3: 55, 5: 22}, 3: {3: 33}}, 2: {2: {3: 44}}},
        )
...
class GroupRowsByBreakdownTest(unittest.TestCase):
    def test_get_breakdown_key(self):
        ...
        self.assertEqual(sort_helper.get_breakdown_key({"account_id": 1, "source_id": 2}, ["account_id"]), (1,))

        self.assertEqual(
            sort_helper.get_breakdown_key({"account_id": 1, "source_id": 2}, ["account_id", "source_id"]), (1, 2)
        )
    ...
    def test_group_rows_by_breakdown_key_breakdown_0(self):
        ...
        self.assertEqual(
            sort_helper.group_rows_by_breakdown_key(
                [], [{"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}, {"account_id": 2, "clicks": 3}]
            ),
            {
                tuple([]): [
    ...
    def test_group_rows_by_breakdown_key_breakdown_1(self):
        ...
        self.assertEqual(
            ...
            collections.OrderedDict(
                [
                    ((2,), [{"account_id": 2, "clicks": 2}, {"account_id": 2, "clicks": 3}]),
                    ((5,), [{"account_id": 5, "clicks": 3}]),
                    (
                        (1,),
                        [
                            {"account_id": 1, "clicks": 1},
    ...
    def test_group_rows_by_breakdown_key_breakdown_2(self):
        self.assertEqual(
            ...
            collections.OrderedDict(
                [
                    ((1, 1), [{"account_id": 1, "source_id": 1, "clicks": 1}]),
                    (
                        (2, 1),
                        [
                            {"account_id": 2, "source_id": 1, "clicks": 3},
                    ...
                    ((1, 2), [{"account_id": 1, "source_id": 2, "clicks": 2}]),
                ]
            ),
        )
    ...
    def test_group_rows_by_breakdown_key_max_1(self):
        ...
        self.assertEqual(
            sort_helper.group_rows_by_breakdown_key(
                ["account_id"], [{"account_id": 1, "clicks": 1}, {"account_id": 2, "clicks": 3}], max_1=True
            ),
            collections.OrderedDict([((1,), {"account_id": 1, "clicks": 1}), ((2,), {"account_id": 2, "clicks": 3})]),
        )
...
class ApplyOffsetLimitTest(unittest.TestCase):
    def test_apply_offset_limit(self):
        self.assertEqual(
            sort_helper.apply_offset_limit(
                [{"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}, {"account_id": 2, "clicks": 3}], 1, 1
            ),
            [{"account_id": 1, "clicks": 2}],
        )
        ...
        self.assertEqual(
            sort_helper.apply_offset_limit(
                [{"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}, {"account_id": 2, "clicks": 3}], 0, 2
            ),
            [{"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}],
        )

        self.assertEqual(
            sort_helper.apply_offset_limit(
                [{"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}, {"account_id": 2, "clicks": 3}], 2, 2
            ),
            [{"account_id": 2, "clicks": 3}],
        )
    ...
    def test_apply_offset_limit_to_breakdown(self):
        self.assertCountEqual(
            ...
            [{"account_id": 1, "clicks": 1}, {"account_id": 2, "clicks": 3}],
        )
        ...
        self.assertIn(
            ...
            [
                [{"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}, {"account_id": 2, "clicks": 3}],
                [{"account_id": 2, "clicks": 3}, {"account_id": 1, "clicks": 1}, {"account_id": 1, "clicks": 2}],
            ],
        )
