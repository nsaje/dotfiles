...
def insert_redirects(content_ads, clickthrough_resolve=True):
    if settings.R1_DEMO_MODE:
        data = {
            str(content_ad.id): {"redirect": {"url": "http://example.com/FAKE"}, "redirectid": "XXXXXXXXXXXXX"}
...
def update_redirects(content_ads):
    data = [{"redirectid": content_ad.redirect_id, "redirect": {"url": content_ad.url}} for content_ad in content_ads]
    if settings.R1_DEMO_MODE:
        return data
...
def upsert_audience(audience):
    try:
        rules = [
            {"id": str(rule.id), "type": rule.type, "value": rule.value}
            for rule in audience.audiencerule_set.all().order_by("pk")
        ]
        source_pixels = [
            {"url": pixel.url, "type": pixel.source_type.type}
            for pixel in audience.pixel.sourcetypepixel_set.all().order_by("pk")
        ]
        ...
        return _call_api_retry(
            settings.R1_CUSTOM_AUDIENCE_API_URL.format(audience_id=audience.id), data=data, method="PUT"
        )
    except Exception:
