...
class SumDivision(aggregates.Aggregate):
    ...
    template = 'CASE WHEN SUM(%(divisor)s) <> 0 THEN SUM(CAST(%(expressions)s AS FLOAT)) / SUM(%(divisor)s) ELSE NULL END'

    def __init__(self, field, divisor, **extra):
        super(SumDivision, self).__init__(
            field,
            divisor=divisor,
...
class IsSumDivisionNull(aggregates.Aggregate):
    function = 'IS_NULL'
    template = 'CASE WHEN SUM(%(divisor)s) IS NULL OR SUM(%(divisor)s) = 0 OR SUM(%(expressions)s) IS NULL THEN 1 ELSE 0 END'
