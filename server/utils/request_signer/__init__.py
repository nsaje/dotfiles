...
def _validate_request(urllib_request):
    ...
    if urllib_request.type != "https" and urllib_request.origin_req_host not in LOCAL_HOSTS:
...
def _validate_key(secret_key):
    ...
    if len(secret_key) < SIGNATURE_KEY_MIN_LEN:
        raise SignatureError("Key too short, min length: {key_len}".format(key_len=SIGNATURE_KEY_MIN_LEN))
...
def sign_urllib_request(urllib_request, secret_key):
    ...
    signature = _get_signature(ts, parsed_selector.path, parsed_selector.query, urllib_request.data, secret_key)
...
def verify_wsgi_request(wsgi_request, secret_keys):
    ...
    """
    header_signature = wsgi_request.META.get(get_wsgi_header_field_name(SIGNATURE_HEADER))
    header_ts = wsgi_request.META.get(get_wsgi_header_field_name(TS_HEADER))
