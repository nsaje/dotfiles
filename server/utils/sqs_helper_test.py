...
class SqsHelperTestCase(TestCase):
    ...
    def test_get_all_messages(self, mock_get_connection):
        ...
        queue_name = "test"
        queue_mock.get_messages.side_effect = [[message1], [message2], [message3], [message4], [message5], []]
        self.maxDiff = None

        messages = sqs_helper.get_all_messages(queue_name)
        self.assertEqual(6, queue_mock.get_messages.call_count)
        self.assertCountEqual([message1, message2, message3, message4, message5], messages)
