...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--min-pacing", dest="min_pacing", default="50.0", help="Min pacing threshold (default 50%)"
        )
        parser.add_argument(
            "--max-pacing", dest="max_pacing", default="150.0", help="Max pacing threshold (default 150%)"
        )
        parser.add_argument("--date", "-d", dest="date", help="Date checked (default today)")
        parser.add_argument(
            "--days-running",
            dest="days_running",
            default=7,
    ...
    def handle(self, *args, **options):
        ...
        date = (
            options["date"] and datetime.datetime.strptime(options["date"], "%Y-%m-%d").date() or datetime.date.today()
        )
        ...
        valid_emails = set(
            user.email for user in zemauth.models.User.objects.get_users_with_perm("can_receive_pacing_email")
        )
        campaigns = {c.pk: c for c in dash.models.Campaign.objects.filter(pk__in=set(a[0] for a in alarms))}
        ...
        for campaign_id, pacing, alert, projections in alarms:
            ...
            self._print(
                "Campaign {} ({}) has {} pacing {}: send to {}".format(
                    campaign.pk, campaign.name, alert, pacing, ", ".join(emails) if emails else "none"
                )
            )
            if not options["send_emails"]:
                continue
            utils.email_helper.send_pacing_notification_email(campaign, emails, pacing, alert, projections)
