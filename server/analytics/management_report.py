...
LIST_REPORT_DISPLAY = {
    "campaigns": lambda obj: Url(_url(["v2", "analytics", "campaign", obj.pk]), obj.get_long_name()).as_html(),
    "accounts": lambda obj: Url(_url(["v2", "analytics", "account", obj.pk]), obj.get_long_name()).as_html(),
    "credits": lambda obj: "{} ({})".format(
        Url(
            obj.account
            and _url(["v2", "analytics", "account", obj.account_id])
            or obj.agency
            and _url(["v2", "analytics", "account", obj.agency.account_set.all().first().pk])
            or "#",
            str(obj),
        ).as_html(),
        _get_bcm_changes("credit", obj),
    ),
    "budgets": lambda obj: "{} ({})".format(
        Url(
            _url(["v2", "analytics", "campaign", obj.campaign_id]),
            "{}, {}".format(str(obj), obj.campaign.account.get_long_name()),
        ).as_html(),
        _get_bcm_changes("budget", obj),
    ),
}
...
def _get_dict_change(d_prev, d_after):
    return {k: (d_prev.get(k), d_after[k]) for k in d_after if d_after[k] != d_prev.get(k)}
...
def _get_bcm_changes(name, obj):
    ...
    return ", ".join(["{}: {} -> {}".format(key, *_format_change(key, chng)) for key, chng in changes.items()])
...
class ReportContext(object):
    def __init__(self, date):
        ...
        self.agency_accounts = set(
            acc.pk
            for acc in dash.models.Account.objects.filter(agency_id__isnull=False).exclude(agency_id__in=OEN_AGENCIES)
        )
        self.agency_campaigns = set(
            camp.pk
            for camp in dash.models.Campaign.objects.filter(account__agency_id__isnull=False).exclude(
                account__agency_id__in=OEN_AGENCIES
            )
        )
        ...
        yesterday_range = {"created_dt__lt": self.date_after, "created_dt__gte": self.date}
        self.yesterday_created = {
            ...
            "budgets": dash.models.BudgetLineItem.objects.filter(**yesterday_range).exclude(
                campaign__account_id__in=OEN_ACCOUNTS
            ),
            "credits": dash.models.CreditLineItem.objects.filter(**yesterday_range)
            .exclude(account_id__in=OEN_ACCOUNTS)
            .exclude(agency_id__in=OEN_AGENCIES),
        }

        yesterday_range = {"modified_dt__lt": self.date_after, "modified_dt__gte": self.date}
    ...
    def _prepare_main_model(self):
        ...
        start_date = datetime.date(
            self.date.year if start_month > 0 else self.date.year - 1, 12 if start_month < 1 else start_month, 1
        )

        statements_qs = (
            dash.models.BudgetDailyStatement.objects.filter(date__gte=start_date, date__lte=self.date)
            .select_related("budget__campaign")
            .select_related("budget__campaign__account")
        )
...
def _get_counts(obj, subset=None, total_only=False):
    ...
    cell = TableCell(
        len(obj["this_month"] & subset),
        info=[_get_change("day", obj, subset), _get_change("week", obj, subset), _get_change("month", obj, subset)]
        if not total_only
        else [],
    )
    return cell
...
def _get_oen(context):
    ...
    projections = analytics.projections.CurrentMonthBudgetProjections(
        "account", accounts=dash.models.Account.objects.filter(pk__in=OEN_ACCOUNTS), campaign_id__in=oen_campaigns
    )
    return [
        TableCell(1),
...
def _populate_agency(context, type_filter):
    valid_accounts = (
        set(account_id for account_id, account_type in context.account_types.items() if account_type == type_filter)
        & context.agency_accounts
    )
    valid_campaigns = (
        set(
            campaign_id for campaign_id, campaign_type in context.campaign_types.items() if campaign_type == type_filter
        )
    ...
    projections = analytics.projections.CurrentMonthBudgetProjections(
        "account", accounts=dash.models.Account.objects.filter(pk__in=valid_accounts), campaign_id__in=monthly_campaigns
    )
    postfix = ""
...
def _prepare_table_rows(context):
    ...
    agency_rows = list(
        map(
            TableRow.prepare(TableRow.TYPE_BREAKDOWN),
            [
                ...
                + _populate_agency(context, dash.constants.AccountType.ACTIVATED),
    ...
    agency_costs = [
        TableRow(
            [
                TableCell("{} agencies".format(_get_counts(context.agencies, total_only=True).value_html())),
    ...
    clientdirect_rows = list(
        map(
            TableRow.prepare(TableRow.TYPE_BREAKDOWN),
            [
                [TableCell("Managed", align="right")]
                + _populate_clientdirect(context, dash.constants.AccountType.MANAGED),
                ...
                [TableCell("Unknown", align="right")]
                + _populate_clientdirect(context, dash.constants.AccountType.UNKNOWN),
            ],
        )
    )
    clientdirect_totals = [
        TableRow(
            [TableCell("Client-direct")] + [_get_totals(clientdirect_rows, i) for i in range(1, 7)],
            row_type=TableRow.TYPE_TOTALS,
        )
    ]
    ...
    grand_totals = [
        TableRow(
            [TableCell("Z1 Total")] + [_get_totals(agency_costs + clientdirect_totals, i) for i in range(1, 7)],
            row_type=TableRow.TYPE_TOTALS,
        ),
        TableRow([TableCell("OEN Total")] + _get_oen(context), row_type=TableRow.TYPE_TOTALS),
    ]

    return header + agency_costs + agency_rows + clientdirect_totals + clientdirect_rows + grand_totals
        ...
        data.append(
            {
                ...
                "elements": [LIST_REPORT_DISPLAY[item](elt) for elt in elements],
            }
        )
...
def get_daily_report_html(date=None):
    ...
    return render_to_string(
        "management_report.html", {"table": _generate_table_html(context), "lists": _generate_lists_html(context)}
    )
