...
def _group_seen_users(date, active_account_ids):
    ...
    user_ids = zemauth.models.User.objects.all().filter(
...
def _prepare_table_rows(date):
    active_account_ids = dash.models.AdGroup.objects.all().filter_active().select_related(
        'campaign'
...
def get_weekly_report_html(date=None):
    ...
    return render_to_string('client_report.html', {
        'table': _generate_table_html(date),
    })
