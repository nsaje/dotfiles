...
class BudgetProjections(object):
    ...
    BREAKDOWN_PREFIX = {"account": "campaign__"}
    FUTURE_METRICS = {
        "account": ("flat_fee", "total_fee", "allocated_total_budget", "allocated_media_budget"),
        "campaign": ("allocated_total_budget", "allocated_media_budget"),
    }
    MANAGED_ACCOUNT_TYPES = (dash.constants.AccountType.MANAGED,)
    ...
    def __init__(self, start_date, end_date, breakdown, projection_date=None, accounts=[], **constraints):
        ...
        self.projection_date = min(projection_date or (self.today - datetime.timedelta(1)), end_date)
        ...
        self.confidence_date = start_date - datetime.timedelta(BudgetProjections.CONFIDENCE_OFFSET_DAYS)
    ...
    def _prepare_budgets(self, constraints):
        self.budgets = (
            ...
            .exclude(end_date__lt=self.confidence_date, start_date__gt=self.end_date)
            .filter(**constraints)
            .select_related("campaign", "campaign__account", "credit")
            .prefetch_related(
                Prefetch(
                    "statements",
                    queryset=dash.models.BudgetDailyStatement.objects.filter(
                        date__gte=self.confidence_date, date__lte=self.projection_date
                    ).order_by("date"),
                )
            )
        )
    ...
    def _prepare_account_types(self):
        self.account_types = {
            ...
            for sett in dash.models.AccountSettings.objects.all()
            .group_current_settings()
            .values("account_id", "account_type")
        }
        self.campaign_types = {
    ...
    def _calculate_totals(self):
        ...
        if self.totals["ideal_media_spend"]:
            self.totals["pacing"] = (
                self.totals["attributed_media_spend"] / self.totals["ideal_media_spend"] * Decimal(100)
            ).quantize(BudgetProjections.PRECISION, rounding=BudgetProjections.ROUNDING)
    ...
    def _calculate_rows(self):
        ...
        for key, budgets in self.calculation_groups.items():
            ...
            num_of_positive_statements = len(
                [
                    x
                    for x in [
            ...
            self._calculate_recognized_fees(row, budgets, key, credit_line_items_map, accounts_with_spend_map)
            self._calculate_media_spend_projection(key, row, budgets, statements_on_date, num_of_positive_statements)
            self._calculate_license_fee_projection(key, row, budgets, statements_on_date, num_of_positive_statements)
    ...
    def _calculate_media_spend_projection(self, key, row, budgets, statements_on_date, num_of_positive_statements):
        assert "allocated_media_budget" in row
        ...
        if self._is_managed(key) and (
            self.first_of_month or num_of_positive_statements <= BudgetProjections.CONFIDENCE_OFFSET_DAYS
        ):
    ...
    def _calculate_recognized_fees(self, row, budgets, account_id, credit_line_items_map, accounts_with_spend_map):
        ...
        ) + self._calculate_agency_credit_flat_fee_share(self.accounts.get(account_id), accounts_with_spend_map)
    ...
    def _calculate_license_fee_projection(self, key, row, budgets, statements_on_date, num_of_positive_statements):
        ...
        if self.end_date < self.today or (
            self._is_managed(key)
            and (self.first_of_month or num_of_positive_statements <= BudgetProjections.CONFIDENCE_OFFSET_DAYS)
        ):
            # skip projection if it's less than the OFFSET, assume allocated budget
        ...
        fee_nano = 0
        for date in utils.dates_helper.date_range(self.start_date, self.projection_date + datetime.timedelta(1)):
            fee_nano += sum(s.license_fee_nano for s in statements_on_date.get(date, []))

        row["license_fee_projection"] = min(
            converters.nano_to_decimal(float(fee_nano) / self.past_days) * Decimal(self.forecast_days), maximum_fee
        )
