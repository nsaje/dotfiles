...
class ManagementReportTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.context.account_types = {1: 1, 2: 1, 3: 2, 4: 3, 5: 3, 6: 4}
        self.context.campaign_types = {1: 2, 2: 2, 3: 3, 4: 4, 5: 5}
    ...
    def test_daily_empty(self):
        ...
        self.assertIn('<div class="section"><table', html)
        self.assertIn(
    ...
    def test_daily_bcm_lists(self):
        ...
        credit = dash.models.CreditLineItem.objects.create_unsafe(
            account_id=1, amount=5000, start_date=today, end_date=today + datetime.timedelta(10), status=1
        )
        dash.models.BudgetLineItem.objects.create_unsafe(
            campaign_id=1, credit=credit, amount=5000, start_date=credit.start_date, end_date=credit.end_date
        )

        html = analytics.management_report.get_daily_report_html(today)
        self.assertIn(
            '<li><a href="https://one.zemanta.com/v2/analytics/account/1">1 - test account 1 - $5000 - ', html
        )
        self.assertIn('<li><a href="https://one.zemanta.com/v2/analytics/campaign/1">$5000', html)

        html = analytics.management_report.get_daily_report_html(today + datetime.timedelta(1))
        self.assertNotIn(
            '<li><a href="https://one.zemanta.com/v2/anlaytics/account/1">1 - test account 1 - $5000', html
        )
    ...
    def test_daily_account_campaign_lists(self):
        ...
        dash.models.Campaign(account=acc, name="Campaign 2").save(r)
        html = analytics.management_report.get_daily_report_html(today)

        self.assertIn(
            '<li><a href="https://one.zemanta.com/v2/analytics/campaign/3">Account Account 2, Campaign Campaign 2</a></li>',
            html,
        )
    ...
    def test_get_totals(self):
        table = [[TableCell(10), TableCell(20)], [TableCell(30), TableCell(50)]]
        self.assertEqual(analytics.management_report._get_totals(table, 0).value, 40)
        self.assertEqual(analytics.management_report._get_totals(table, 1).value, 70)
    ...
    def test_get_change(self):
        ...
        self.assertEqual(analytics.management_report._get_change("month", accounts, set([1, 2, 3, 4, 5])), -2)
        self.assertEqual(analytics.management_report._get_change("week", accounts, set([1, 2, 3, 4, 5])), 2)
        self.assertEqual(analytics.management_report._get_change("week", accounts, set([3, 5])), -1)
    ...
    def test_get_counts(self):
        ...
        self.assertEqual(cell.as_html(), "<td>3 (<b>+1</b>, <b>+2</b>, <b>-2</b>)</td>")
        self.assertEqual(cell.value, 3)
        self.assertEqual(cell.info, [1, 2, -2])

        cell = analytics.management_report._get_counts(accounts, set([1, 2, 3, 4, 5]), total_only=True)
        self.assertEqual(cell.as_html(), "<td>3</td>")
