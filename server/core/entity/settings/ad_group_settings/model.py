...
class AdGroupSettings(
    ...
    b1_sources_group_cpc_cc = models.DecimalField(
        max_digits=10, decimal_places=4, default=core.source.AllRTBSourceType.min_cpc, verbose_name="Bidder's Bid CPC"
    )
    ...
    def _get_human_value_for_retargeting_adgroups(value):
        if not value:
            ...
            names = core.entity.AdGroup.objects.filter(pk__in=value).values_list("name", flat=True)
            value = ", ".join(names)
        return value
