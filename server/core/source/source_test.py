...
class CanManageContentAds(TestCase):
    def test_filter_can(self):
        ...
        self.assertEqual(
            test_helper.QuerySetMatcher(core.source.Source.objects.all().filter_can_manage_content_ads()), [source]
        )

    def test_filter_cannot(self):
        source = magic_mixer.blend(
            core.source.Source, maintenance=False, deprecated=False, source_type__available_actions=[]
        )

        self.assertFalse(source.can_manage_content_ads())
        self.assertEqual(
            test_helper.QuerySetMatcher(core.source.Source.objects.all().filter_can_manage_content_ads()), []
        )
