...
class EntityLimitEnforceTest(TestCase):
    def setUp(self):
        ...
        self.ACCOUNT_EXCEPTIONS = {
            'AdGroup': {
                self.ACCOUNT_WITH_EXCEPTION: 3,
            },
        }

        self.default_limits_patcher = mock.patch(
            'core.common.entity_limits.DEFAULT_LIMITS',
            self.DEFAULT_LIMITS,
        )
        ...
        self.account_exceptions_patcher = mock.patch(
            'core.common.entity_limits.ACCOUNT_EXCEPTIONS',
            self.ACCOUNT_EXCEPTIONS,
        )
