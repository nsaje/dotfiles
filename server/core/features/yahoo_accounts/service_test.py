...
class YahooFinalizeMigrationTestCase(django.test.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.source_yahoo = magic_mixer.blend(
            dash.models.Source, source_type=source_type)
    ...
    def test_finalize_migration(self, mock_k1_helper, mock_today, mock_update_account, mock_update_ad_group, mock_update_content_ad):
    ...
    def test_finalize_migration_wrong_status(self, mock_k1_helper):
        mock_k1_helper.get_yahoo_migration.return_value = {
            'status': constants.MIGRATION_STATUS_SYNCING_TO_BOTH,
        }
        with self.assertRaises(service.CannotRunMigrationException):
            service.finalize_migration(self.account.id)
    ...
    def test_update_source_campaign_keys(self):
        yahoo_ad_group_sources = magic_mixer.cycle(2).blend(
            ...
            ad_group__campaign__account=self.account, source=self.source_yahoo,
            source_campaign_key='test-old')
        other_ad_group_source = magic_mixer.blend(
            dash.models.AdGroupSource,
            ad_group__campaign__account=self.account, source=self.source_other,
            source_campaign_key='test-old')
        other_account_ad_group_source = magic_mixer.blend(
            dash.models.AdGroupSource,
            ad_group__campaign__account=self.other_account, source=self.source_yahoo,
            source_campaign_key='test-old')

        mapping = {
            yahoo_ad_group_sources[0].ad_group_id: 'test-new',
        }
    ...
    def test_update_source_content_ad_ids(self):
        yahoo_content_ad_sources = magic_mixer.cycle(2).blend(
            ...
            content_ad__ad_group__campaign__account=self.account, source=self.source_yahoo,
            source_content_ad_id='test-old')
        other_content_ad_source = magic_mixer.blend(
            dash.models.ContentAdSource,
            content_ad__tracker_urls=[],
            content_ad__ad_group__campaign__account=self.account, source=self.source_other,
            source_content_ad_id='test-old')
        other_account_content_ad_source = magic_mixer.blend(
            dash.models.ContentAdSource,
            content_ad__tracker_urls=[],
            content_ad__ad_group__campaign__account=self.other_account, source=self.source_yahoo,
            source_content_ad_id='test-old')

        mapping = {
            yahoo_content_ad_sources[0].content_ad_id: 'test-new',
        }
