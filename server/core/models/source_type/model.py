...
class SourceType(models.Model, bcm_mixin.SourceTypeBCMMixin):
    ...
    def get_min_cpc(self, ad_group_settings, bcm_modifiers=None):
        ...
        if self.type == constants.SourceType.YAHOO and ad_group_settings.target_devices == [
    ...
    def can_update_daily_budget_manual(self):
        return (
            ...
            and constants.SourceAction.CAN_UPDATE_DAILY_BUDGET_MANUAL in self.available_actions
        )

    def can_update_daily_budget_automatic(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_UPDATE_DAILY_BUDGET_AUTOMATIC in self.available_actions
        )

    def can_manage_content_ads(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_MANAGE_CONTENT_ADS in self.available_actions
        )

    def has_3rd_party_dashboard(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.HAS_3RD_PARTY_DASHBOARD in self.available_actions
        )

    def can_modify_start_date(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_MODIFY_START_DATE in self.available_actions
        )

    def can_modify_end_date(self):
        return (
            self.available_actions is not None and constants.SourceAction.CAN_MODIFY_END_DATE in self.available_actions
        )

    def can_modify_device_targeting(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_MODIFY_DEVICE_TARGETING in self.available_actions
        )
    ...
    def can_modify_ad_group_name(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_MODIFY_AD_GROUP_NAME in self.available_actions
        )

    def can_modify_ad_group_iab_category_automatic(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_MODIFY_AD_GROUP_IAB_CATEGORY_AUTOMATIC in self.available_actions
        )

    def can_modify_ad_group_iab_category_manual(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_MODIFY_AD_GROUP_IAB_CATEGORY_MANUAL in self.available_actions
        )

    def update_tracking_codes_on_content_ads(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.UPDATE_TRACKING_CODES_ON_CONTENT_ADS in self.available_actions
        )
    ...
    def can_fetch_report_by_publisher(self):
        return (
            self.available_actions is not None
            and constants.SourceAction.CAN_FETCH_REPORT_BY_PUBLISHER in self.available_actions
        )
