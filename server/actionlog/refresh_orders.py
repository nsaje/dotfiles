...
def refresh_fetch_all_orders():
    ...
    last_success_times_not_none = filter(last_success_times, lambda x: x is not None)
    ...
    statsd_helper.statsd_gauge('actionlog.num_ags_not_synced', len(filter(last_success_times, lambda x: x is None)))
