...
class Command(ExceptionCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "accounts", nargs="?", help="Comma separated list of account ids."
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def _validate_account_ids(self, account_ids, filter_only=False):
        ...
        invalid_account_ids = [
            ...
            if adgroup_counts.get(acc_id, 0) > MAX_AD_GROUP_COUNT_LIMIT
        ]
        if not filter_only and invalid_account_ids:
            raise CommandError(
                "Some accounts have too many ad groups: "
                + ", ".join(str(acc_id) for acc_id in invalid_account_ids)
            )
        ...
        if data:
            ...
            raise CommandError(
                "Some accounts have impressions in RS: {}".format(
                    ", ".join(
                        [
                            "account {} - {}".format(acc_id, spend)
                            for acc_id, spend in data.items()
                        ]
                    )
                )
            )
    ...
    def _get_unused_archived_accounts_ids(self):
        youngest_change_date = datetime.datetime.now() - datetime.timedelta(
            ARCHIVED_AT_LEAST_DAYS
        )
        candidates = set(
            acc.pk
            for acc in Account.objects.filter(
                outbrain_marketer_id__isnull=False
            ).exclude(outbrain_marketer_id="")
            if acc.adgroupsettings_set.all().count()
            and acc.get_current_settings().created_dt < youngest_change_date
        )
        unarchived_accounts = set(
            acc.pk for acc in Account.objects.all().exclude_archived()
        )
        accounts_with_spend = set(
            acc.pk for acc in Account.objects.all().filter_with_spend()
        )

        return self._validate_account_ids(
            candidates - accounts_with_spend - unarchived_accounts, filter_only=True
        )
    ...
    def _release_accounts(self, account_ids):
        ...
        for account_id in account_ids:
            ...
            ad_group_sources = AdGroupSource.objects.filter(
                source=OB, ad_group__campaign__account=account
            )
            if not account.outbrain_marketer_id:
                self.stdout.write(
                    "Missing marketer id for account {} {}".format(
            ...
            ob_accounts = OutbrainAccount.objects.filter(
                marketer_id=account.outbrain_marketer_id
            )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "Marketers: {}".format(", ".join([str(mid) for mid in marketer_ids]))
        )
