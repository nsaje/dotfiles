...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options.get('ad_groups'):
            lookup['content_ad__ad_group_id__in'] = map(int, options.get('ad_groups', '').split(','))
        elif options.get('campaigns'):
            lookup['content_ad__ad_group__campaign_id__in'] = map(int, options.get('campaigns', '').split(','))
        else:
