...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--not-exclusive", dest="not_exclusive", action="store_true", default=False
        )
    ...
    def _clean_ids(self, raw):
        return (
            raw
            and set(map(int, [x.strip() for x in raw.split(",") if x.strip()]))
            or set()
        )
    ...
    def handle(self, *args, **options):
        ...
        if source_id.isdigit():
            ...
            source = dash.models.Source.objects.get(
                Q(bidder_slug="b1_{}".format(source_id)) | Q(bidder_slug=source_id)
            )
        if not source:
        ...
        with transaction.atomic():
            deal, created = dash.models.DirectDeal.objects.get_or_create(
            ...
            for ad_group in ad_groups:
                ddc = dash.models.DirectDealConnection.objects.create(
                    source=source,
                    exclusive=not options["not_exclusive"],
                    adgroup=ad_group,
                )
                ddc.deals.add(deal)
