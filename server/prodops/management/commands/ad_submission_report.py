...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['is_pending']:
            cas_list = cas_list.filter(
                submission_status__in=(dash.constants.ContentAdSubmissionStatus.PENDING,
                                       dash.constants.ContentAdSubmissionStatus.NOT_SUBMITTED, )
            )
            flags.append('pending')
        if options['is_live']:
            cas_list = cas_list.filter(
                content_ad__ad_group_id__in=dash.models.AdGroup.objects.all().filter_running().values_list('pk', flat=True)
            )
            flags.append('live')
