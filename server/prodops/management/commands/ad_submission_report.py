...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--premium-only", dest="is_premium", action="store_true", default=False
        )
        parser.add_argument(
            "--not-premium", dest="is_not_premium", action="store_true", default=False
        )
        parser.add_argument(
            "--approved-only", dest="is_approved", action="store_true", default=False
        )
        parser.add_argument(
            "--pending-only", dest="is_pending", action="store_true", default=False
        )
        parser.add_argument(
            "--live-only", dest="is_live", action="store_true", default=False
        )
        parser.add_argument(
            "--ex-oen", dest="is_not_oen", action="store_true", default=False
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["is_premium"]:
            cas_list = cas_list.filter(
                ...
                | Q(
                    content_ad__ad_group__campaign__account__custom_flags__b1_msn_approved=True
                )
                | Q(
                    content_ad__ad_group__campaign__account__agency__custom_flags__b1_msn_approved=True
                )
            )
        ...
        self._print(
            ...
            + prodops.helpers.generate_report(
                "{}_ads_{}_{}".format(source, "-".join(flags), datetime.date.today()),
                [HEADER] + out,
            )
        )
