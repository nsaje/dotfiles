...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--active-only', dest='is_active', action='store_true', default=False)
    ...
    def handle(self, *args, **options):
        ...
        if source.isdigit():
            ...
            cas_list = cas_list.filter(source=dash.models.Source.objects.get(
        ...
        if options['is_premium']:
            cas_list = cas_list.filter(
                content_ad__ad_group__campaign_id__in=dash.models.CustomHack.objects.filter(
                    summary='Approved for msn.com',
        ...
        if options['is_pending']:
            cas_list = cas_list.filter(
                submission_status__in=(dash.constants.ContentAdSubmissionStatus.PENDING, dash.constants.ContentAdSubmissionStatus.NOT_SUBMITTED, )
            )
            flags.append('pending')
        if options['is_active']:
            cas_list = cas_list.filter(
                content_ad__ad_group_id__in=dash.models.AdGroup.objects.all().filter_active().values_list('pk', flat=True)
            )
            flags.append('active')
        if options['is_not_oen']:
            cas_list = cas_list.exclude(
                content_ad__ad_group__campaign__account_id=OEN
            )
            flags.append('exoen')
        ...
        self._print('Report: ' + prodops.helpers.generate_report('{}_ads_{}_{}'.format(
            source,
            '-'.join(flags),
            datetime.date.today()
        ), [HEADER] + out))
