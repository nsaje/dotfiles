...
class Command(utils.command_helpers.ExceptionCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--search', dest='search', default=None,
                            help='Search string in history')
        parser.add_argument('--action-type', dest='action_type', default=None,
                            help='Specific action type. Types: ' + ', '.join(
                                '{} - {}'.format(k, v) for k, v in dash.constants.HistoryActionType._get_all_kv_pairs()
                            ))
    ...
    def handle(self, *args, **options):
        ...
        start_date = (options['start_date'] and
                      datetime.datetime.strptime(options['start_date'], "%Y-%m-%d").date() or
        ...
        history = dash.models.History.objects.filter(
            created_dt__range=(datetime.datetime.combine(start_date, datetime.time.min),
        ...
        for entry in history:
            if prev_agency != entry.agency or prev_account != entry.account \
               or prev_campaign != entry.campaign or prev_ad_group != entry.ad_group:
                self._print(' - {}, {}, {}, {},'.format(
                    entry.agency or '/', entry.account, entry.campaign, entry.ad_group
                ))
            self._print('    - {} {} - {} ({}): {}'.format(
                entry.created_dt,
                entry.created_by,
            ...
            prev_agency, prev_account, prev_campaign, prev_ad_group = entry.agency, entry.account, entry.campaign, entry.ad_group
