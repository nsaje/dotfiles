...
class MVHSourceTest(TestCase, backtosql.TestSQLMixin):
    ...
    def test_generate(self, mock_s3helper, mock_transaction, mock_cursor):
        mv = materialize_views.MVHelpersSource(
            "asd", datetime.date(2016, 7, 1), datetime.date(2016, 7, 3), account_id=None
        )
...
class MVHCampaignFactorsTest(TestCase, backtosql.TestSQLMixin):
    ...
    def test_generate(self, mock_s3helper, mock_transaction, mock_cursor):
        mv = materialize_views.MVHelpersCampaignFactors(
            "asd", datetime.date(2016, 7, 1), datetime.date(2016, 7, 2), account_id=None
        )
...
class MVHCurrencyExchangeRatesTest(TestCase, backtosql.TestSQLMixin):
    ...
    def test_generate(self, mock_s3helper, mock_transaction, mock_cursor):
        ...
        mv = materialize_views.MVHelpersCurrencyExchangeRates(
...
class MasterViewTest(TestCase, backtosql.TestSQLMixin):
    ...
    def test_generate_rows(self):
        ...
        with mock.patch.object(
            materialize_views.MasterView, "get_postclickstats", return_value=postclickstats_return_value
        ):
            mv = materialize_views.MasterView(
                "asd", datetime.date(2016, 7, 1), datetime.date(2016, 7, 3), account_id=None
            )
...
class MVConversionsTest(TestCase, backtosql.TestSQLMixin):
    ...
    def test_generate_rows(self):
        ...
        with mock.patch.object(
            ...
            mv = materialize_views.MVConversions(
                "asd", datetime.date(2016, 7, 1), datetime.date(2016, 7, 3), account_id=None
            )
            rows = list(mv.generate_rows(mock_cursor, date))
...
class ReplicasTest(TestCase, backtosql.TestSQLMixin):
    ...
    def test_prepare_unload_query(self):
        ...
        sql, params = materialize_views.prepare_unload_csv_query("mypath/", "mv_master", from_date, to_date)
