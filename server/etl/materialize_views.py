...
def prepare_unload_csv_query(s3_path, table_name, date_from, date_to, account_id=None):
    ...
    return sql, {"s3_url": s3_url, "credentials": credentials, "delimiter": CSV_DELIMITER}
...
class MVHelpersSource(Materialize):
    ...
    def generate(self, **kwargs):
        s3_path = upload_csv(self.TABLE_NAME, self.date_to, self.job_id, self.generate_rows)
...
class MVHelpersCampaignFactors(Materialize):
    ...
    def generate(self, campaign_factors, **kwargs):
        ...
        s3_path = upload_csv(self.TABLE_NAME, self.date_to, self.job_id, partial(self.generate_rows, campaign_factors))
        ...
        for date in rrule.rrule(rrule.DAILY, dtstart=self.date_from, until=self.date_to):
            ...
            with db.get_write_stats_transaction():
                with db.get_write_stats_cursor() as c:
                    ...
                    s3_path = upload_csv(self.TABLE_NAME, date, self.job_id, partial(self.generate_rows, c, date))
    ...
    def generate(self, **kwargs):
        with db.get_write_stats_transaction():
            with db.get_write_stats_cursor() as c:
                ...
                sql, params = prepare_date_range_delete_query(
