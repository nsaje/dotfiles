...
class Command(Z1Command):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--date_from", help="Iso formatted date. All publisher group entries after this date will be ignored"
        )
    ...
    def check_ad_groups(self, ad_groups, publishers_map, stats_map, date_from, log_influx=True):
        ...
        ad_group_settings_map = {
            x.ad_group_id: x
            for x in models.AdGroupSettings.objects.filter(ad_group__in=ad_groups).group_current_settings()
        }
        campaign_settings_map = {
            x.campaign_id: x
            for x in models.CampaignSettings.objects.filter(campaign__in=campaigns).group_current_settings()
        }
        account_settings_map = {
            x.account_id: x
            for x in models.AccountSettings.objects.filter(account__in=accounts).group_current_settings()
        }
        agency_settings_map = {
            x.agency_id: x for x in models.AgencySettings.objects.filter(agency__in=agencies).group_current_settings()
        }
        ...
        for ad_group in ad_groups:
            blacklist, whitelist = publisher_group_helpers.concat_publisher_group_targeting(
                ...
                ad_group.campaign,
                campaign_settings_map[ad_group.campaign_id],
            ...
            overall_blacklisted_entry_ids |= set(
                models.PublisherGroupEntry.objects.filter(publisher_group_id__in=blacklist)
                .filter(modified_dt__lt=date_from)
                .values_list("id", flat=True)
            )
            # do not filter out recent, because we can already have some data for whitelisted
            overall_whitelisted_entry_ids |= set(
                models.PublisherGroupEntry.objects.filter(publisher_group_id__in=whitelist).values_list("id", flat=True)
            )
    ...
    def get_whitelist_violators(self, publisher_ids, whitelisted_publishers, blacklisted_publishers):
        ...
        return set(
            [
                ...
                if publisher_id not in lookup and publisher_id != "all publishers__4"
            ]
        )
    ...
    def get_blacklist_violators(self, publisher_ids, blacklisted_publishers):
        ...
        return set(
            [
                publisher_id
                for publisher_id in publisher_ids
                if publisher_id in lookup and publisher_id != "all publishers__4"
            ]
        )
    ...
    def get_stats(self, date_from, date_to, ad_groups):
        ...
        for row in results:
            if row["publisher_id"] not in m[row["ad_group_id"]]:
                m[row["ad_group_id"]][row["publisher_id"]] = {"clicks": 0, "impressions": 0}
