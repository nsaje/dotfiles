...
def refresh_k1_reports(update_since, account_id=None, skip_vacuum=False, skip_analyze=False):
    ...
    _refresh_k1_reports(
        update_since, MATERIALIZED_VIEWS, account_id, skip_vacuum=skip_vacuum, skip_analyze=skip_analyze
    )
    if do_post_to_slack or account_id:
...
def _refresh_k1_reports(update_since, views, account_id=None, skip_vacuum=False, skip_analyze=False):
    ...
    logger.info(
        "Starting refresh k1 reports job %s for date range %s - %s, requested update since %s, %s",
        job_id,
        date_from,
        date_to,
        update_since,
