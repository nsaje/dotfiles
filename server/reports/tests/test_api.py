...
class UpsertReportsTestCase(test.TestCase):
    ...
    def test_save_reports(self):
        ...
        agn1 = dashmodels.AdGroupSource.objects.get(id=1)
        rows_agn1_date1 = [
            {
        ...
        agn2 = dashmodels.AdGroupSource.objects.get(id=1)
        rows_agn1_date2 = [
            {
                'title': 'Test Article 1',
                'url': 'http://example.com/',
        ...
        api.save_report(agn1.ad_group, agn1.source, rows_agn1_date1, date1)
        ...
        api.save_report(agn2.ad_group, agn2.source, rows_agn2_date1, date1)
        ...
        api.save_report(agn1.ad_group, agn1.source, rows_agn1_date2, date2)
        api.save_report(agn2.ad_group, agn2.source, rows_agn2_date2, date2)
    ...
    def test_save_reports_reinsert(self):
        ...
        agn = dashmodels.AdGroupSource.objects.get(id=1)
        date = datetime.date(2014, 7, 10)

        stats = models.ArticleStats.objects.filter(ad_group=agn.ad_group, source=agn.source, datetime=date)
        self.assertEqual(len(stats), 0)

        api.save_report(agn.ad_group, agn.source, rows, date)
        stats = models.ArticleStats.objects.filter(ad_group=agn.ad_group, source=agn.source, datetime=date)
        ...
        api.save_report(agn.ad_group, agn.source, rows_new_title, date)
        stats = models.ArticleStats.objects.filter(ad_group=agn.ad_group, source=agn.source, datetime=date)
        self.assertEqual(len(stats), 2)
        ...
        api.save_report(agn.ad_group, agn.source, rows_new_url, date)
        self.assertEqual(len(stats), 2)
