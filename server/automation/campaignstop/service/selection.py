...
def _get_max_spend_grouped_by_adg(ad_groups_map, adg_spends):
    ...
    for adg_id, adg_data in ad_groups_map.items():
        ...
        settings_budget = float(adg_data["settings__daily_budget"]) or 0
        spend += max(settings_budget, todays_realtime_spend)
    return spend
...
def _log_differences(
    campaign, ad_groups_map, adg_sources_qs, adg_source_spends, adg_spends, adg_source_calculation, adg_calculation
):
    logger.warning(
        LOG_MESSAGE,
        campaign=campaign,
        ad_groups_map=ad_groups_map,
        adg_sources_qs=adg_sources_qs,
        adg_source_spends=adg_source_spends,
        adg_spends=adg_spends,
