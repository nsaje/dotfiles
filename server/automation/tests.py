...
class BudgetDepletionTestCase(test.TestCase):
    ...
    def fake_send_depleted_budget_notification_email(first_name, email, campaign_name):
        return

    @patch("automation.budgetdepletion.send_depleted_budget_notification_email", fake_send_depleted_budget_notification_email)
    def test_notify_campaign_with_depleting_budget(self):
        budgetdepletion.notify_campaign_with_depleting_budget(models.Campaign.objects.get(pk=1), 100, 150)
        notif = automationmodels.CampaignBudgetDepletionNotifaction.objects.all().latest('created_dt')
    ...
    @patch("automation.budgetdepletion.send_depleted_budget_notification_email", fake_send_depleted_budget_notification_email)
    def test_manager_has_been_notified(self):
    ...
    def test_send_depleted_budget_notification_email(self):
        budgetdepletion.send_depleted_budget_notification_email('first_name', 'manager@user.com', 'campaign_name')
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].from_email, 'Zemanta <{}>'.format(settings.DEPLETING_CAMPAIGN_BUDGET_EMAIL))
        self.assertEqual(mail.outbox[0].to, ['manager@user.com'])
