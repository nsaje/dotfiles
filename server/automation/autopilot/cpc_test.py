...
class AutopilotCpcTestCase(test.TestCase):
    ...
    @patch(
        ...
        (
            {"underspend_upper_limit": -1, "underspend_lower_limit": -0.5, "bid_cpc_proc_increase": Decimal("0.1")},
            {"underspend_upper_limit": -0.5, "underspend_lower_limit": -0.1, "bid_cpc_proc_increase": Decimal("0.5")},
            {"underspend_upper_limit": -0.1, "underspend_lower_limit": 0, "bid_cpc_proc_increase": Decimal("-0.5")},
        ),
    )
    ...
    def test_calculate_new_autopilot_cpc(self):
        test_cases = (
            ...
            ("0", "10", "5", "0.1", [CpcChangeComment.CPC_NOT_SET, CpcChangeComment.CURRENT_CPC_TOO_LOW]),
            ("0.5", "10", "8", "0.75", []),
            ("2.5", "10", "8", "2.75", []),
            ("0.5", "10", "10", "0.25", []),
            ("0.5", "10", "2", "0.55", []),
            ...
            ("0.5", "10", "-5", "0.5", []),
            ("-0.5", "10", "5", "0.1", [CpcChangeComment.CPC_NOT_SET, CpcChangeComment.CURRENT_CPC_TOO_LOW]),
            ("0.35", "10", "9.96", "0.15", []),
            ("2.8", "10", "9.96", "2.5", []),
        ...
        for test_case in test_cases:
            self.assertEqual(
                cpc.calculate_new_autopilot_cpc(Decimal(test_case[0]), Decimal(test_case[1]), Decimal(test_case[2])),
    ...
    def test_calculate_new_autopilot_cpc_automatic_mode_rtb(self):
        test_cases = (
            ...
            ("0", "10", "5", "2", 1.0, "0.01", [CpcChangeComment.CPC_NOT_SET, CpcChangeComment.CURRENT_CPC_TOO_LOW]),
            ("0.5", "10", "8", "2", 1.0, "0.75", []),
            ("2.5", "10", "8", "2", 1.0, "2.75", []),
            ...
            ("0.5", "10", "-5", "2", 1.0, "0.55", []),
            ("-0.5", "10", "5", "2", 1.0, "0.01", [CpcChangeComment.CPC_NOT_SET, CpcChangeComment.CURRENT_CPC_TOO_LOW]),
    ...
    def test_get_calculate_cpc_comments(self):
        ...
        for test_case in test_cases:
            self.assertEqual(
                cpc._get_calculate_cpc_comments(Decimal(test_case[0]), Decimal(test_case[1]), Decimal(test_case[2])),
                (Decimal(test_case[3]), test_case[4]),
            )
    ...
    def test_threshold_reducing_cpc(self):
        ...
        for test_case in test_cases:
            self.assertEqual(
                cpc._threshold_reducing_cpc(Decimal(test_case[0]), Decimal(test_case[1])), Decimal(test_case[2])
            )
    ...
    def test_threshold_increasing_cpc(self):
        ...
        for test_case in test_cases:
            self.assertEqual(
                cpc._threshold_increasing_cpc(Decimal(test_case[0]), Decimal(test_case[1])), Decimal(test_case[2])
            )
    ...
    def test_threshold_source_constraints(self, mock_source_type_min_max_cpc):
        ...
        for test_case in test_cases:
            comments = []
            self.assertEqual(
                cpc._threshold_source_constraints(
                    Decimal(test_case[0]),
                    ags_type,
                    ag_settings,
                    comments,
    ...
    def test_get_source_type_min_max_cpc(self):
        ...
        for test_case in test_cases:
            self.assertEqual(
                cpc._get_source_type_min_max_cpc(test_case[0], ag_settings, bcm_modifiers),
    ...
    def test_round_cpc(self):
        ...
        for test_case in test_cases:
            self.assertEqual(
                cpc._round_cpc(Decimal(test_case[0]), decimal_places=2, rounding=decimal.ROUND_HALF_UP),
                Decimal(test_case[1]),
            )
    ...
    def test_threshold_ad_group_constraints(self):
        ...
        for test_case in test_cases:
            comments = []
            self.assertEqual(
                cpc._threshold_ad_group_constraints(Decimal(test_case[0]), adgroup, comments, 3), Decimal(test_case[1])
            )
            self.assertEqual(comments, test_case[2])
