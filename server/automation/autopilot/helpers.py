...
def send_autopilot_changes_email(campaign, emails, changes_data, bcm_modifiers):
    ...
    for adgroup, adgroup_changes in changes_data.items():
        media_sources, media_sources_url = _get_email_adgroup_pausing_suggestions(
            adgroup, adgroup_changes, bcm_modifiers
        )
        changes.append(
            {
                "adgroup": adgroup,
    ...
    try:
        email_helper.send_official_email(
            ...
            **email_helper.params_from_template(template, **args)
        )
    except Exception:
        logger.exception(
            "Autopilot e-mail for campaign %s to %s was not sent" + "because an exception was raised:",
            campaign.name,
            ", ".join(emails),
        )
...
def send_budget_autopilot_initialisation_email(campaign, emails, changes_data):
    ...
    try:
        ...
        logger.exception(
            "Autopilot e-mail for initialising budget autopilot on an adroup in "
...
def _get_email_adgroup_pausing_suggestions(adgroup, adgroup_changes, bcm_modifiers):
    ...
    for ag_source in adgroup_changes:
        ...
        if all(b in changes for b in ["new_budget", "old_budget"]) and changes[
            "new_budget"
        ] == get_ad_group_sources_minimum_daily_budget(ag_source, bcm_modifiers):
...
def _get_budget_changes_text(budget_changed, changes):
    if budget_changed:
        return "daily spend cap changed from ${} to ${}".format(
            "{0:.2f}".format(changes["old_budget"]), "{0:.2f}".format(changes["new_budget"])
        )
    elif DailyBudgetChangeComment.NEW_BUDGET_NOT_EQUAL_DAILY_BUDGET in changes["budget_comments"]:
        return "daily spend cap did not change because " + DailyBudgetChangeComment.get_text(
            DailyBudgetChangeComment.NEW_BUDGET_NOT_EQUAL_DAILY_BUDGET
        )
    else:
...
def _get_cpc_changes_text(cpc_changed, changes):
    if cpc_changed:
        text = "bid CPC changed from ${} to ${}".format(
            changes["old_cpc_cc"].normalize(), changes["new_cpc_cc"].normalize()
        )
    ...
    return "bid CPC remained unchanged at ${}.".format(changes["old_cpc_cc"].normalize())
