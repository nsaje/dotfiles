...
def _apply_rules(
    ...
    logger.info(
        f"Applying rules of target type {constants.TargetType.get_name(target_type)} on {len(rules_map)} ad groups.",
        target_type=constants.TargetType.get_name(target_type).lower(),
    )
    with utils.metrics_compat.block_timer(
        "automation.rules.execute.apply_rules", target_type=constants.TargetType.get_name(target_type).lower()
    ):
        _apply_rules_inner(
            target_type,
            rules_map,
...
def _prefetch_data(target_type: int, rules_map: Dict[core.models.AdGroup, List[models.Rule]]):
    logger.info(
        f"Prefetching data for target type {constants.TargetType.get_name(target_type)} on {len(rules_map)} ad groups.",
        target_type=constants.TargetType.get_name(target_type).lower(),
    )
    with utils.metrics_compat.block_timer(
        "automation.rules.execute.prefetch", target_type=constants.TargetType.get_name(target_type).lower()
    ):
        return _prefetch_data_inner(target_type, rules_map)


def _prefetch_data_inner(target_type: int, rules_map: Dict[core.models.AdGroup, List[models.Rule]]):
