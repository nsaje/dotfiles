...
def narrow_filtered_sources(sources, ad_group_sources):
    ...
    sources = sources.filter(pk__in=list(ad_group_sources.distinct("source_id").values_list("source_id", flat=True)))
...
def prepare_all_accounts_constraints(
    user,
    start_date,
    end_date,
    filtered_sources,
    ...
    constraints = {"allowed_accounts": allowed_accounts, "allowed_campaigns": allowed_campaigns}
    ...
    add_publisher_constraints(
        constraints, filtered_sources, show_blacklisted_publishers, None, None, None, None, None, None
    )
...
@newrelic.agent.function_trace()
def prepare_account_constraints(
    user,
    account,
    start_date,
    end_date,
    ...
    add_publisher_constraints(
        constraints, filtered_sources, show_blacklisted_publishers, None, None, None, None, account, account_settings
    )
...
@newrelic.agent.function_trace()
def prepare_campaign_constraints(
    user,
    campaign,
    start_date,
    end_date,
    filtered_sources,
    filtered_ad_groups=None,
    show_archived=False,
    only_used_sources=True,
    show_blacklisted_publishers=dash.constants.PublisherBlacklistFilter.SHOW_ALL,
):
    ...
    constraints = {
        ...
        "allowed_ad_groups": allowed_ad_groups,
        "allowed_content_ads": models.ContentAd.objects.filter(ad_group__in=allowed_ad_groups).exclude_archived(
            show_archived
        ),
    }
...
@newrelic.agent.function_trace()
def prepare_ad_group_constraints(
    user,
    ad_group,
    start_date,
    end_date,
    filtered_sources,
    filtered_content_ads=None,
    show_archived=False,
    only_used_sources=True,
    show_blacklisted_publishers=dash.constants.PublisherBlacklistFilter.SHOW_ALL,
):
...
def add_publisher_constraints(
    ...
    blacklisted_entries = models.PublisherGroupEntry.objects.filter(
        publisher_group_id__in=blacklists
    ).filter_by_sources(filtered_sources, include_wo_source=True)
    whitelisted_entries = models.PublisherGroupEntry.objects.filter(
...
def narrow_allowed_target_field(constraints, breakdown):
    ...
    if parent_dimension == "account_id":
        ...
        ad_group_sources = models.AdGroupSource.objects.filter(ad_group__campaign__in=constraints["allowed_campaigns"])
        ...
        ad_group_sources = models.AdGroupSource.objects.filter(ad_group__in=constraints["allowed_ad_groups"])
...
def get_uses_bcm_v2(user, constraints, level):
    ...
    if level == dash.constants.Level.ALL_ACCOUNTS:
        ...
        accessible_non_bcm_accounts = (
            models.Account.objects.filter_by_user(user).exclude_archived().filter(uses_bcm_v2=False).exists()
        )
