...
def _get_or_create_device(device_key):
    ...
    try:
        return models.Device.objects.get(
            device_key=device_key,
            expire_date__gte=dates_helper.utc_today(),
        )
...
def _create_device_entry():
    for _ in range(MAX_RETRIES):
        device = models.Device(
            device_key=_get_new_device_key(),
            expire_date=_get_expire_date(),
        )
        try:
            device.save()
            return device
