...
class PublisherBlacklistTest(restapi.common.views_base_test.RESTAPITest):
    ...
    def test_adgroups_publishers_put_unlist(self):
        ...
        publisher_group_helpers.blacklist_publishers(self.test_request, [{'publisher': 'cnn2.com', 'source': source, 'include_subdomains': True}], self.test_ad_group.campaign)
        publisher_group_helpers.blacklist_publishers(self.test_request, [{'publisher': 'cnn3.com', 'source': source, 'include_subdomains': True}], self.test_ad_group.campaign.account)
    ...
    def test_modifiers_set(self):
        ...
        r = self.client.put(
            reverse('publishers_list', kwargs={'ad_group_id': self.test_ad_group.id}),
            data=test_put, format='json')
        resp_json = self.assertResponseValid(r, data_type=list)
        self.assertEqual(resp_json['data'], test_put)

        self.assertEqual(
            core.publisher_bid_modifiers.get(self.test_ad_group),
            [
                {
                    'publisher': 'testpub1',
                    'source': source,
