...
class BidModifierCSVTest(restapi.common.views_base_test_case.RESTAPITestCase):
    ...
    @mock.patch("utils.s3helpers.S3Helper.put")
    @mock.patch("core.features.bid_modifiers.helpers.create_csv_error_key")
    def test_upload_validation_unset_when_limit_exceeded(self, mock_create_csv_error_key, mock_s3_helper_put):
        ad_group = magic_mixer.blend(core.models.AdGroup, campaign__account=self.account)
        ...
        csv_columns = [target_column_name, "Source Slug", "Bid Modifier"]
        entries = [
            {target_column_name: "whatever", "Source Slug": self.source.bidder_slug, "Bid Modifier": "2.5"},
            {target_column_name: bms[0].target, "Source Slug": self.source.bidder_slug, "Bid Modifier": None},
        ]
        csv_writer = csv.DictWriter(csv_file, csv_columns)
        csv_writer.writeheader()
        csv_writer.writerows(entries)
        ...
        self.assertEqual(response.status_code, 200)
        result = self.assertResponseValid(response)

        self.assertEqual(
            result,
            {
                "data": {
                    "deleted": {"count": 5001, "dimensions": 1, "summary": [{"type": "PUBLISHER", "count": 5001}]},
                    "created": {"count": 1, "dimensions": 1, "summary": [{"type": "PUBLISHER", "count": 1}]},
                }
            },
        )
...
class BidModifierTypeSummariesTest(restapi.common.views_base_test_case.RESTAPITestCase):
    def setUp(self):
