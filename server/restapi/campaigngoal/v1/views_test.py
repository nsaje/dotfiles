...
class CampaignGoalsTest(RESTAPITest):
    ...
    def test_campaigngoals_get(self):
        r = self.client.get(
            reverse("restapi.campaigngoal.v1:campaigngoals_details", kwargs={"campaign_id": 608, "goal_id": 1238})
        )
        resp_json = self.assertResponseValid(r)
        self.validate_against_db(resp_json["data"])

    def test_campaigngoals_list(self):
        r = self.client.get(reverse("restapi.campaigngoal.v1:campaigngoals_list", kwargs={"campaign_id": 608}))
    ...
    def test_campaigngoals_post(self):
        ...
        del post_data["id"]
        r = self.client.post(
            reverse("restapi.campaigngoal.v1:campaigngoals_list", kwargs={"campaign_id": 608}),
            data=post_data,
            format="json",
        )
