...
class AccountCreditRefundTest(RESTAPITest):
    ...
    def test_refund_get(self):
        r = self.client.get(
            reverse('accounts_credits_refunds_details', kwargs={
                'account_id': 186,
                'credit_id': 861,
                'refund_id': 777,
            })
    ...
    def test_account_credits_list(self):
        r = self.client.get(reverse('accounts_credits_refunds_list', kwargs={
            'account_id': 186,
            'credit_id': 861,
        }))
    ...
    def test_account_credits_list_all(self):
        r = self.client.get(reverse('accounts_credits_refunds_list_all', kwargs={
            'account_id': 186,
        }))
        resp_json = self.assertResponseValid(r, data_type=list)
        for item in resp_json['data']:
