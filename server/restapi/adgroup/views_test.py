...
class AdGroupsTest(RESTAPITest):
    ...
    def adgroup_repr(
        ...
        representation = {
            ...
            'endDate': end_date if end_date else cls.expected_none_output,
            'maxCpc': Decimal(max_cpc).quantize(Decimal('1.0000')) if max_cpc else cls.expected_none_output,
            'maxCpm': Decimal(max_cpm).quantize(Decimal('1.0000')) if max_cpm else cls.expected_none_output,
            'dailyBudget': Decimal(daily_budget).quantize(Decimal('1.0000')) if daily_budget else cls.expected_none_output,
            ...
            'autopilot': {
                ...
                'dailyBudget': Decimal(autopilot_daily_budget).quantize(Decimal('1.0000')) if autopilot_daily_budget else cls.expected_none_output,
            },
            'dayparting': dayparting,
            'deliveryType': constants.AdGroupDeliveryType.get_name(delivery_type),
            'clickCappingDailyAdGroupMaxClicks': click_capping_daily_ad_group_max_clicks if click_capping_daily_ad_group_max_clicks else cls.expected_none_output,
            'clickCappingDailyClickBudget': click_capping_daily_click_budget if click_capping_daily_click_budget else cls.expected_none_output,
        }
    ...
    def test_adgroups_invalid_bluekai_targeting(self):
        ...
        adgroup = self.adgroup_repr(audience_targeting=restapi.serializers.targeting.AudienceSerializer(demographic_targeting).data)
        r = self.client.put(
            reverse('adgroups_details', kwargs={'ad_group_id': 2040}),
            data=adgroup,
            format='json',
    ...
    def test_adgroups_get_permissioned(self):
        ...
        resp_json['data']['clickCappingDailyAdGroupMaxClicks'] = self.expected_none_output
        resp_json['data']['clickCappingDailyClickBudget'] = self.expected_none_output
        self.validate_against_db(resp_json['data'])
    ...
    def test_adgroups_put_blank_strings(self):
        ...
        resp_json = self.assertResponseValid(r)
        self.validate_against_db(resp_json['data'])
        self.assertEqual(resp_json['data']['endDate'], self.expected_none_output)
        self.assertEqual(resp_json['data']['maxCpc'], self.expected_none_output)
        self.assertEqual(resp_json['data']['maxCpm'], self.expected_none_output)
        ...
        self.assertEqual(resp_json['data']['dailyBudget'], self.expected_none_output)
        self.assertEqual(resp_json['data']['autopilot']['dailyBudget'], self.expected_none_output)
        self.assertEqual(resp_json['data']['clickCappingDailyAdGroupMaxClicks'], self.expected_none_output)
        self.assertEqual(resp_json['data']['clickCappingDailyClickBudget'], self.expected_none_output)
