...
class AdGroupSourcesTest(RESTAPITest):
    ...
    def test_adgroups_sources_put(self):
        ...
        r = self.client.put(
            reverse('adgroups_sources_list', kwargs={'ad_group_id': ad_group.id}),
            test_ags,
            format='json',
        )
        resp_json = self.assertResponseValid(r, data_type=list)
    ...
    def test_adgroups_sources_rounding(self, min_cpc_mock, min_daily_budget_mock, max_daily_budget_mock):
        ...
        ad_group_sources = magic_mixer.blend(
            dash.models.AdGroupSource,
            ad_group=ad_group,
            source__bidder_slug=('a',),
        )
        # min cpc - would return 0.12 without rounding ceiling
        test_ags = [
            self.adgroupsource_repr(
                source=ad_group_sources.source.bidder_slug,
                cpc='0.1200',
            ),
        ]
        r = self.client.put(
