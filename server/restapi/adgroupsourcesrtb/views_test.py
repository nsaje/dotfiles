...
class AdGroupSourcesRTBTest(RESTAPITest):
    ...
    def test_adgroups_sources_rtb_rounding(self, min_cpc_mock, min_daily_budget_mock, max_daily_budget_mock):
        ...
        ad_group.settings.update_unsafe(None, cpc_cc=0.7792, autopilot_state=constants.AdGroupSettingsAutopilotState.INACTIVE)

        # min cpc - would return 0.12 without rounding ceiling
        test_agsr = self.adgroupsourcertb_repr(
            cpc='0.1200',
        )
        r = self.client.put(
            reverse('adgroups_sources_rtb_details', kwargs={'ad_group_id': ad_group.id}),
            test_agsr,
            format='json',
        )
