...
class GeolocationListViewTest(restapi.common.views_base_test_case.RESTAPITestCase):
    ...
    def test_get_with_keys_filter(self):
        keys = ["US", "GB"]
        r = self.client.get(reverse("restapi.geolocation.v1:geolocation_list") + "?keys=US,GB")
        r = self.assertResponseValid(r, data_type=list)
        self.assertFalse(any(location["key"] not in keys for location in r["data"]))

    def test_get_with_types_filter(self):
        types = ["COUNTRY", "CITY"]
        r = self.client.get(reverse("restapi.geolocation.v1:geolocation_list") + "?types=COUNTRY,CITY")
        r = self.assertResponseValid(r, data_type=list)
        self.assertFalse(any(location["type"] not in types for location in r["data"]))

    def test_get_with_name_contains_filter(self):
        r = self.client.get(reverse("restapi.geolocation.v1:geolocation_list") + "?nameContains=New")
        r = self.assertResponseValid(r, data_type=list)
    ...
    def test_get_with_types_and_name_contains_filter(self):
        types = ["COUNTRY", "CITY"]
        r = self.client.get(reverse("restapi.geolocation.v1:geolocation_list") + "?types=COUNTRY,CITY&nameContains=New")
        r = self.assertResponseValid(r, data_type=list)
    ...
    def test_get_with_offset_and_limit(self):
        r = self.client.get(reverse("restapi.geolocation.v1:geolocation_list") + "?offset=2&limit=2")
        r = self.assertResponseValid(r, data_type=list)
