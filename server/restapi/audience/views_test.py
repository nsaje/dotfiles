...
class AudiencesTest(RESTAPITest):
    ...
    def test_audience_get(self):
        ...
        r = self.client.get(reverse("audiences_details", kwargs={"account_id": account.id, "audience_id": audience.id}))
    ...
    def test_audiences_list(self):
        ...
        r = self.client.get(reverse("audiences_list", kwargs={"account_id": account.id}))
    ...
    def test_post_blank(self):
        account = magic_mixer.blend(core.models.Account, users=[self.user])
        r = self.client.post(reverse("audiences_list", kwargs={"account_id": account.id}), data={}, format="json")
    ...
    def test_put(self):
        ...
        old_id = audience.id
        r = self.client.put(
            reverse("audiences_details", kwargs={"account_id": account.id, "audience_id": audience.id}),
            data={
                "id": old_id + 10,
                "pixel_id": pixel2.id,
                "name": "putty",
    ...
    def test_put_audience_does_not_exist(self):
        account = magic_mixer.blend(core.models.Account, users=[self.user])
        r = self.client.put(
            reverse("audiences_details", kwargs={"account_id": account.id, "audience_id": 1000}),
            data={"name": "new name"},
            format="json",
        )
        self.assertResponseError(r, "MissingDataError")
