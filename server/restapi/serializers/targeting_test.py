...
class OsSerializerTests(TestCase):
    def test_deserialization(self):
        data = {"name": "IOS", "version": {"min": "IOS_8_0", "max": "IOS_9_0"}}
        serializer = targeting.OSSerializer(data=data)
        serializer.is_valid()
        self.assertEqual(serializer.validated_data, {"name": "ios", "version": {"min": "ios_8_0", "max": "ios_9_0"}})

    def test_serialization(self):
        data = {"name": "ios", "version": {"min": "ios_8_0", "max": "ios_9_0"}}
        serializer = targeting.OSSerializer(data)
        self.assertEqual(serializer.data, {"name": "IOS", "version": {"min": "IOS_8_0", "max": "IOS_9_0"}})

    def test_validation_version_missmatch(self):
        data = {"name": "IOS", "version": {"min": "IOS_9_0", "max": "IOS_8_0"}}

        serializer = targeting.OSSerializer(data=data)
        self.assertFalse(serializer.is_valid())

    def test_validation_compatibility(self):
        data = {"name": "IOS", "version": {"min": "WINDOWS_XP", "max": "IOS_8_0"}}
...
class DemographicSerializerTests(TestCase):
    def setUp(self):
        self.serialized = {
            "AND": [
                {"OR": [{"category": "bluekai:12345"}, {"category": "lotame:23456"}, {"category": "lr-test:34567"}]},
                {"OR": [{"category": "outbrain:45678"}, {"category": "lotame:56789"}]},
                {"OR": [{"category": "obs:98765"}, {"category": "obi:98765"}]},
                {"NOT": [{"category": "bluekai:98765"}]},
            ]
        }
        self.deserialized = [
            "and",
    ...
    def test_deserialization(self):
        ...
        self.assertTrue(serializer.is_valid())
        self.assertEqual(serializer.validated_data, self.deserialized)
    ...
    def test_serialization_error(self):
        data = {"AND": [{"XOR": [{"category": "12345"}, {"category": "23456"}, {"category": "34567"}]}]}
        serializer = targeting.AudienceSerializer(data=data)
        self.assertFalse(serializer.is_valid())
...
class TestTargetRegions(TestCase):
    ...
    def test_valid_single(self):
        ...
        serializer.is_valid(raise_exception=True)
        self.assertEqual(serializer.validated_data, ["US"])
    ...
    def test_us_territories(self):
        ...
        serializer.is_valid(raise_exception=True)
        self.assertEqual(serializer.validated_data, ["PR"])
