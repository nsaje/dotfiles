    ...
    def test_create_parents(self):
        self.assertEqual(
            helpers.create_parents(
                [{"source_id": 3, "status": 1, "name": "asd"}, {"source_id": 4, "status": 1, "name": "qwe"}],
                ["source_id"],
            ),
            [{"source_id": [3, 4]}],
        )
    ...
    def test_get_all_dimensions(self):
        self.assertCountEqual(
            helpers.get_all_dimensions(
                ...
                {"date": datetime.date(2016, 7, 7), "content_ad_id": 1},
                [{"source_id": 3}],
            ),
        ...
        self.assertCountEqual(
            helpers.get_all_dimensions(
                ["account_id"], {"date": datetime.date(2016, 7, 7), "content_ad_id": 1}, [{"source_id": 3}]
            ),
    ...
    def test_select_relevant_stats_rows(self):
        self.assertCountEqual(
            helpers.select_relevant_stats_rows(
                ["account_id"],
                [{"account_id": 1}, {"account_id": 2}, {"account_id": 4}],
                [{"account_id": 1, "clicks": 1}, {"account_id": 2, "clicks": 2}, {"account_id": 3, "clicks": 3}],
            ),
            [{"account_id": 1, "clicks": 1}, {"account_id": 2, "clicks": 2}],
        )


class PrepareTimeConstraintsTest(TestCase):
    def test_dimension_day(self):
        constraints = {"date__gte": datetime.date(2016, 2, 1), "date__lte": datetime.date(2016, 2, 16)}

        self.assertEqual(
            helpers.get_time_dimension_constraints(constants.TimeDimension.DAY, constraints, 0, 5),
            {"date__gte": datetime.date(2016, 2, 1), "date__lte": datetime.date(2016, 2, 5)},
        )

        self.assertEqual(
            helpers.get_time_dimension_constraints(constants.TimeDimension.DAY, constraints, 10, 3),
            {"date__gte": datetime.date(2016, 2, 11), "date__lte": datetime.date(2016, 2, 13)},
        )
    ...
    def test_dimension_week(self):
        ...
        self.assertEqual(
            ...
            {"date__gte": datetime.date(2016, 2, 1), "date__lte": datetime.date(2016, 2, 14)},
        )

        self.assertEqual(
            helpers.get_time_dimension_constraints(constants.TimeDimension.WEEK, constraints, 1, 1),
            {"date__gte": datetime.date(2016, 2, 8), "date__lte": datetime.date(2016, 2, 14)},
        )

        self.assertEqual(
            helpers.get_time_dimension_constraints(constants.TimeDimension.WEEK, constraints, 1, 2),
            {"date__gte": datetime.date(2016, 2, 8), "date__lte": datetime.date(2016, 2, 16)},  # should use end date
        )

    def test_dimension_month(self):
        constraints = {"date__gte": datetime.date(2016, 2, 5), "date__lte": datetime.date(2016, 4, 16)}

        self.assertEqual(
            helpers.get_time_dimension_constraints(constants.TimeDimension.MONTH, constraints, 0, 2),
            {"date__gte": datetime.date(2016, 2, 5), "date__lte": datetime.date(2016, 3, 31)},
        )

        self.assertEqual(
            helpers.get_time_dimension_constraints(constants.TimeDimension.MONTH, constraints, 2, 3),
            {"date__gte": datetime.date(2016, 4, 1), "date__lte": datetime.date(2016, 4, 16)},
        )
