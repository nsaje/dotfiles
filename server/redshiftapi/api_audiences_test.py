...
class AudiencesTestCase(TestCase):
    ...
    def test_get_audience_sample_size(self, mock_dictfetchall, mock_get_cursor):
        ...
        expected_query = "SELECT COUNT(DISTINCT(zuid)) FROM pixie_sample WHERE account_id = %s AND slug = %s AND timestamp > %s " "AND (referer ILIKE %s + '%%' OR referer ILIKE %s + '%%' OR referer ILIKE '%%' + %s + '%%' OR referer " "NOT ILIKE %s + '%%' OR referer NOT ILIKE %s + '%%')"
