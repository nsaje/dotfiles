    ...
    def test_prepare_query(self):
        ...
        FROM
            (SELECT a.ad_group_id AS ad_group_id,
                    ...
                    (CASE WHEN SUM(a.visits) <> 0 THEN SUM(CAST(a.total_time_on_site AS FLOAT)) / SUM(a.visits)
                        ELSE NULL END) avg_tos,
                    (SUM(a.effective_cost_nano)+SUM(a.effective_data_cost_nano)+SUM(a.license_fee_nano))/
                        1000000000.0 billing_cost,
                    (CASE WHEN SUM(a.visits) <> 0 THEN SUM(CAST(a.bounced_visits AS FLOAT)) / SUM(a.visits)
                        ELSE NULL END)*100.0 bounce_rate,
                    (CASE WHEN SUM(a.clicks) = 0 THEN NULL WHEN SUM(a.visits) = 0 THEN 1 WHEN
                        SUM(a.clicks) < SUM(a.visits) THEN 0
                        ELSE (SUM(CAST(a.clicks AS FLOAT)) - SUM(a.visits)) / SUM(a.clicks) END)*100.0
                        click_discrepancy,
                    SUM(a.clicks) clicks,
                    SUM(a.cost_nano)/1000000000.0 cost,
                    (CASE WHEN SUM(a.clicks) <> 0 THEN SUM(CAST(a.cost_nano AS FLOAT)) / SUM(a.clicks) ELSE
                        NULL END)/1000000000.0 cpc,
                    (CASE WHEN SUM(a.impressions) <> 0 THEN SUM(CAST(a.clicks AS FLOAT)) / SUM(a.impressions) ELSE
                        NULL END)*100.0 ctr,
                    ...
                    (CASE WHEN SUM(a.visits) <> 0 THEN SUM(CAST(a.new_visits AS FLOAT)) / SUM(a.visits) ELSE NULL END)
                         *100.0 percent_new_users,
                    (CASE WHEN SUM(a.visits) <> 0 THEN SUM(CAST(a.pageviews AS FLOAT)) / SUM(a.visits) ELSE NULL END)
                         pv_per_visit,
                    (SUM(a.license_fee_nano)+SUM(a.cost_nano)+SUM(a.data_cost_nano))/1000000000.0 total_cost,
                    SUM(a.visits) visits,
                    ROW_NUMBER() OVER (PARTITION BY a.ad_group_id
                                        ORDER BY a.campaign_id DESC) AS r
