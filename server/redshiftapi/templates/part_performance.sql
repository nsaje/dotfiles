...
CASE
  WHEN
      {{ is_cost_dependent }}
      AND TRUNC(CAST(COALESCE({{ cost_column }}, 0) AS DECIMAL(22, 18)), 2) > 0  -- has cost
      AND COALESCE(
            TRUNC(CAST(CASE WHEN {{ has_conversion_key }} THEN {{ cost_column }} / NULLIF({{ conversion_key }}, 0) ELSE {{ metric_column }} END AS DECIMAL(22, 18)), {{ metric_val_decimal_places }})  -- metric value
          , 0) = 0 -- no spend
