...
class SQLMatcher:
    ...
    def __eq__(self, other):
        try:
            if not isinstance(other, str) and "obj" in other.__dict__:
...
class ColumnTestCase(TestCase, TestSQLMixin):
    def test_only_column(self):
        ...
        self.assertEqual(column.only_alias(), "py_cat")
        self.assertEqual(column.only_alias(prefix="t"), "t.py_cat")
    ...
    def test_set_group(self):
        ...
        self.assertEqual(column.group, 1)


class TemplateColumnTestCase(TestCase, TestSQLMixin):
    def test_only_column(self):
        column = backtosql.TemplateColumn("test_col.sql", {"column_name": "cat", "multiplier": 100}, alias="py_cat")
    ...
    def test_no_alias(self):
        column = backtosql.TemplateColumn("test_col.sql", {"column_name": "cat", "multiplier": 100})
    ...
    def test_strip_comments(self):
        column = backtosql.TemplateColumn(
            "test_col_comment.sql", {"column_name": "cat", "multiplier": 100}, alias="py_cat"
        )
        self.assertSQLEquals(column.column_as_alias(), "SUM(cat)*100 AS py_cat")
        self.assertSQLEquals(column.column_as_alias("t"), "SUM(t.cat)*100 AS py_cat")
...
class OrderColumnTestCase(TestCase, TestSQLMixin):
    ...
    def test_get_direction(self):
        self.assertEqual(helpers.get_order("-cat"), "DESC")
        self.assertEqual(helpers.get_order("cat"), "ASC")
        self.assertEqual(helpers.get_order("+cat"), "ASC")

    def test_get_direction_nulls(self):
        self.assertEqual(helpers.get_order("-cat", nulls="last"), "DESC NULLS LAST")
        self.assertEqual(helpers.get_order("cat", nulls="first"), "ASC NULLS FIRST")
        self.assertEqual(helpers.get_order("+cat", nulls=None), "ASC")
...
class ModelTestCase(TestCase):
    ...
    def test_init_columns_alias(self):
        self.assertEqual(self.ModelA.py_foo.alias, "py_foo")
        self.assertEqual(self.ModelA.py_bar.alias, "py_bar")
        self.assertEqual(self.ModelA.py_cat.alias, "py_cat")
        self.assertEqual(self.ModelA.py_dog.alias, "py_dog")

    def test_get_columns(self):
        self.assertCountEqual(
            self.ModelA().get_columns(),
            [self.ModelA.py_foo, self.ModelA.py_bar, self.ModelA.py_cat, self.ModelA.py_dog],
        )

    def test_get_column(self):
        self.assertEqual(self.ModelA().get_column("py_foo"), self.ModelA.py_foo)

    def test_select_columns(self):
        m = self.ModelA()

        self.assertCountEqual(m.select_columns(), m.get_columns())

        self.assertCountEqual(m.select_columns(subset=["py_foo", "py_bar"]), [m.py_foo, m.py_bar])

        self.assertCountEqual(m.select_columns(group=1), [m.py_foo, m.py_cat])

        self.assertCountEqual(m.select_columns(subset=["py_foo", "py_bar"], group=1), [m.py_foo])

        self.assertCountEqual(m.select_columns(subset=None), [m.py_foo, m.py_bar, m.py_cat, m.py_dog])
        self.assertCountEqual(m.select_columns(subset=[]), [])

        with self.assertRaises(backtosql.BackToSQLException):
            # should raise exception if nonexistent column is specified
            self.assertCountEqual(m.select_columns(subset=["nonexistent"]), [m.py_foo])
    ...
    def test_add_column(self):
        ...
        self.assertEqual(m.food, column)

        with self.assertRaises(AttributeError):
            getattr(self.ModelA, "food")

        self.assertCountEqual(m.select_columns(subset=["py_foo", "py_bar", "food"]), [m.py_foo, m.py_bar, m.food])

        self.assertCountEqual(m.select_columns(group=1), [m.py_foo, m.py_cat, m.food])
...
class HelpersTestCase(TestCase):
    def test_clean_alias(self):
        self.assertEqual(helpers.clean_alias("+cat"), "cat")
        self.assertEqual(helpers.clean_alias("-cat"), "cat")
        self.assertEqual(helpers.clean_alias("cat"), "cat")

    def test_clean_prefix(self):
        self.assertEqual(helpers.clean_prefix("t"), "t.")
        self.assertEqual(helpers.clean_prefix("t."), "t.")
